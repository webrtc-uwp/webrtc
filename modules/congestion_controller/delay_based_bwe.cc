...
DelayBasedBwe::DelayBasedBwe(RtcEventLog* event_log, const Clock* clock)
    : event_log_(event_log),
      ...
      prev_logged_bitrate_(0),
      prev_logged_state_(BandwidthUsage::kBwNormal),
...
DelayBasedBwe::Result DelayBasedBwe::MaybeUpdateEstimate(
  ...
  if ((result.updated && prev_logged_bitrate_ != result.target_bitrate_bps) ||
      detector_.State() != prev_logged_state_) {
    uint32_t bitrate_bps =
        result.updated ? result.target_bitrate_bps : prev_logged_bitrate_;
    ...
    prev_logged_bitrate_ = bitrate_bps;
    prev_logged_state_ = detector_.State();
  }
  return result;
}
