...
class MouseCursorMonitorMac : public MouseCursorMonitor {
 ...
 private:
  ...
  rtc::scoped_refptr<FullScreenChromeWindowDetector> full_screen_chrome_window_detector_;
};
...
MouseCursorMonitorMac::MouseCursorMonitorMac(const DesktopCaptureOptions& options,
    ...
    : configuration_monitor_(options.configuration_monitor()),
      ...
      full_screen_chrome_window_detector_(options.full_screen_chrome_window_detector()) {
  assert(window_id == kCGNullWindowID || screen_id == kInvalidScreenId);
  if (screen_id != kInvalidScreenId && rtc::GetOSVersionName() < rtc::kMacOSLion) {
...
void MouseCursorMonitorMac::Capture() {
  ...
  if (window_id_ != kCGNullWindowID) {
    ...
    if (window_array) {
      ...
      for (CFIndex i = 0; i < count; ++i) {
        ...
        if (window_name && CFStringCompare(window_name, CFSTR("Dock"), 0) == 0) continue;
        ...
        if (window_bounds && window_number) {
          ...
          DesktopRect window_rect = DesktopRect::MakeXYWH(gc_window_rect.origin.x,
                                                          gc_window_rect.origin.y,
                                                          gc_window_rect.size.width,
                                                          gc_window_rect.size.height);

          CGWindowID window_id;
          if (!CFNumberGetValue(window_number, kCFNumberIntType, &window_id)) continue;

          if (window_id == on_screen_window) {
            found_window = true;
            if (!window_rect.Contains(position)) state = OUTSIDE;
    ...
    if (screen_id_ != kFullDesktopScreenId) {
      ...
      const MacDisplayConfiguration* config =
          configuration.FindDisplayConfigurationById(static_cast<CGDirectDisplayID>(screen_id_));
      if (config) {
        if (!config->pixel_bounds.Contains(position)) state = OUTSIDE;
  ...
  if (state == INSIDE) {
    ...
    position = DesktopVector(round(position.x() * scale), round(position.y() * scale));
  }
...
void MouseCursorMonitorMac::CaptureImage(float scale) {
  ...
  DesktopSize size(round(nssize.width * scale), round(nssize.height * scale));  // Pixel size
