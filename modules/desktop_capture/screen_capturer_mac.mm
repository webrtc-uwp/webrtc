...
void ScreenCapturerMac::CaptureFrame() {
  ...
  *new_frame->mutable_updated_region() = region;
  ...
  if (flip) new_frame.reset(new InvertedDesktopFrame(std::move(new_frame)));
...
void ScreenCapturerMac::GlBlitFast(const DesktopFrame& frame, const DesktopRegion& region) {
...
void ScreenCapturerMac::CgBlitPreLion(const DesktopFrame& frame, const DesktopRegion& region) {
...
bool ScreenCapturerMac::CgBlitPostLion(const DesktopFrame& frame, const DesktopRegion& region) {
  // Copy the entire contents of the previous capture buffer, to capture over.
  // TODO(wez): Get rid of this as per crbug.com/145064, or implement
  // crbug.com/92354.
  if (queue_.previous_frame()) {
