...
class FileAudioStream : public AudioStreamInterface {
 public:
  FileAudioStream(size_t num_callbacks, const std::string& file_name, int sample_rate)
    ...
    const size_t num_16bit_samples = test::GetFileSize(file_name) / kBytesPerSample;
    file_.reset(new int16_t[num_16bit_samples]);
    FILE* audio_file = fopen(file_name.c_str(), "rb");
    EXPECT_NE(audio_file, nullptr);
    size_t num_samples_read = fread(file_.get(), sizeof(int16_t), num_16bit_samples, audio_file);
  ...
  void Read(void* destination, size_t num_frames) override {
    memcpy(destination, static_cast<int16_t*>(&file_[file_pos_]), num_frames * sizeof(int16_t));
    file_pos_ += num_frames;
  }

  int file_size_in_seconds() const {
    return static_cast<int>(file_size_in_bytes_ / (kBytesPerSample * sample_rate_));
  }
  size_t file_size_in_callbacks() const { return file_size_in_seconds() * kNumCallbacksPerSecond; }
...
class FifoAudioStream : public AudioStreamInterface {
 public:
  ...
  size_t average_size() const {
    return (total_written_elements_ == 0) ? 0.0 :
                                            0.5 +
            static_cast<float>(total_written_elements_) / (write_count_ - kNumIgnoreFirstCallbacks);
  }
...
class LatencyMeasuringAudioStream : public AudioStreamInterface {
 public:
  ...
  void Write(const void* source, size_t num_frames) override {
    ...
    int index_of_max = std::distance(vec.begin(), std::find(vec.begin(), vec.end(), max));
  ...
  int average_latency() const {
    if (latencies_.empty()) return 0;
    return 0.5 +
        static_cast<double>(std::accumulate(latencies_.begin(), latencies_.end(), 0)) /
        latencies_.size();
  }
  ...
  int IndexToMilliseconds(double index) const { return 10.0 * (index / frames_per_buffer_) + 0.5; }
...
class MockAudioTransportIOS : public test::MockAudioTransport {
 public:
  explicit MockAudioTransportIOS(int type)
      : num_callbacks_(0), type_(type), play_count_(0), rec_count_(0), audio_stream_(nullptr) {}
  ...
  void HandleCallbacks(EventWrapper* test_is_done,
    ...
    if (play_mode()) {
      ON_CALL(*this, NeedMorePlayData(_, _, _, _, _, _, _, _))
          .WillByDefault(Invoke(this, &MockAudioTransportIOS::RealNeedMorePlayData));
    }
    if (rec_mode()) {
      ON_CALL(*this, RecordedDataIsAvailable(_, _, _, _, _, _, _, _, _, _))
          .WillByDefault(Invoke(this, &MockAudioTransportIOS::RealRecordedDataIsAvailable));
    }
  }
...
class AudioDeviceTest : public ::testing::Test {
 protected:
  AudioDeviceTest() : test_is_done_(EventWrapper::Create()) {
    ...
    EXPECT_EQ(0, audio_device()->GetPlayoutAudioParameters(&playout_parameters_));
  ...
  rtc::scoped_refptr<AudioDeviceModule> audio_device() const { return audio_device_; }
  ...
  rtc::scoped_refptr<AudioDeviceModule> CreateAudioDevice(
    ...
    rtc::scoped_refptr<AudioDeviceModule> module(AudioDeviceModule::Create(0, audio_layer));
    return module;
  }

  // Returns file name relative to the resource root given a sample rate.
  std::string GetFileName(int sample_rate) {
    EXPECT_TRUE(sample_rate == 48000 || sample_rate == 44100 || sample_rate == 16000);
    char fname[64];
    snprintf(fname, sizeof(fname), "audio_device/audio_short%d", sample_rate / 1000);
    ...
    const int seconds = static_cast<int>(bytes / (sample_rate * kBytesPerSample));
...
TEST_F(AudioDeviceTest, DISABLED_StartPlayoutOnTwoInstances) {
  ...
  EXPECT_CALL(mock,
              NeedMorePlayData(playout_frames_per_10ms_buffer(),
                               kBytesPerSample,
                               playout_channels(),
                               playout_sample_rate(),
                               NotNull(),
...
TEST_F(AudioDeviceTest, StartRecordingVerifyCallbacks) {
  ...
  mock.HandleCallbacks(test_is_done_.get(), nullptr, kNumCallbacks);
  EXPECT_CALL(mock,
              RecordedDataIsAvailable(NotNull(),
...
TEST_F(AudioDeviceTest, RunPlayoutWithFileAsSource) {
  ...
  mock.HandleCallbacks(test_is_done_.get(), file_audio_stream.get(), num_callbacks);
  // SetMaxPlayoutVolume();
  EXPECT_EQ(0, audio_device()->RegisterAudioCallback(&mock));
  StartPlayout();
  test_is_done_->Wait(kTestTimeOutInMilliseconds);
...
TEST_F(AudioDeviceTest, RunPlayoutAndRecordingInFullDuplex) {
  ...
  mock.HandleCallbacks(
      test_is_done_.get(), fifo_audio_stream.get(), kFullDuplexTimeInSec * kNumCallbacksPerSecond);
  // SetMaxPlayoutVolume();
  EXPECT_EQ(0, audio_device()->RegisterAudioCallback(&mock));
  StartRecording();
  StartPlayout();
...
TEST_F(AudioDeviceTest, DISABLED_MeasureLoopbackLatency) {
  ...
  EXPECT_EQ(latency_audio_stream->num_latency_values(),
            static_cast<size_t>(kImpulseFrequencyInHz * kMeasureLatencyTimeInSec - 1));
