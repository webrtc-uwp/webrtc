...
int32_t AudioDeviceWindowsCore::SetWaveOutVolume(uint16_t volumeLeft,
...
int32_t AudioDeviceWindowsCore::WaveOutVolume(uint16_t& volumeLeft,
...
int32_t AudioDeviceWindowsCore::SpeakerVolumeStepSize(
...
int32_t AudioDeviceWindowsCore::MicrophoneVolumeStepSize(
    uint16_t& stepSize) const {
  if (!_microphoneIsInitialized) {
    return -1;
  }
...
int32_t AudioDeviceWindowsCore::SetPlayoutBuffer(
...
int32_t AudioDeviceWindowsCore::PlayoutBuffer(
...
Exit:
  ...
  if (keepPlaying) {
    ...
    _playError = 1;
    LOG(LS_ERROR) << "kPlayoutError message posted: rendering thread has ended"
                  << " pre-maturely";
  } else {
      ...
      if (SUCCEEDED(hr)) {
        ...
        while (syncBufIndex >= _recBlockSize) {
          if (_ptrAudioBuffer) {
            ...
            // Basically what we do here is to measure how many percent of our
            // 10ms period is used for encoding and decoding. This value shuld
            // be used as a warning indicator only and not seen as an absolute
            // value. Running at ~100% will lead to bad QoS.
            time = (int)(t2.QuadPart - t1.QuadPart);
            _avgCPULoad = (float)(_avgCPULoad * .99 +
                                  (time + _playAcc) /
                                      (double)(_perfCounterFreq.QuadPart));
            _playAcc = 0;
