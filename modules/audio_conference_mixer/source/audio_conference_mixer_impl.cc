...
MixerParticipant::MixerParticipant() : _mixHistory(new MixHistory()) {}
...
MixHistory::MixHistory() : _isMixed(0) {}
...
AudioConferenceMixer::Frequency AudioConferenceMixerImpl::OutputFrequency()
    const {
  rtc::CritScope cs(&_crit);
...
int32_t AudioConferenceMixerImpl::SetMixabilityStatus(
  ...
  size_t numMixedParticipants;
  {
    ...
    if (!(mixable ^ isMixed)) {
      LOG(LS_ERROR) << "Mixable is aready " << (isMixed ? "ON" : "off");
      return -1;
    }
    bool success = false;
    if (mixable) {
...
int32_t AudioConferenceMixerImpl::SetMinimumMixingFrequency(Frequency freq) {
  ...
  if ((freq == kNbInHz) || (freq == kWbInHz) || (freq == kSwbInHz) ||
    ...
    LOG(LS_ERROR) << "SetMinimumMixingFrequency incorrect frequency: " << freq;
    assert(false);
    return -1;
  }
}
...
void AudioConferenceMixerImpl::UpdateToMix(
  ...
  LOG(LS_VERBOSE) << "UpdateToMix(mixList,rampOutList,mixParticipantList,"
  ...
  for (MixerParticipantList::const_iterator participant =
    ...
    if (ret == MixerParticipant::AudioFrameInfo::kError) {
      LOG(LS_WARNING) << "failed to GetAudioFrameWithMuted() from participant";
...
bool AudioConferenceMixerImpl::RemoveParticipantFromList(
    MixerParticipant* participant,
    MixerParticipantList* participantList) const {
  LOG(LS_VERBOSE) << "RemoveParticipantFromList(participant, participantList)";
