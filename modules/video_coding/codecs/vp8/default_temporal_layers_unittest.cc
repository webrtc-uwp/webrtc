...
TEST(TemporalLayersTest, 2Layers) {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    EXPECT_TRUE(checker.CheckOnFrameEncoded(i == 0, vp8_info, tl_config));
...
TEST(TemporalLayersTest, KeyFrame) {
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    EXPECT_TRUE(checker.CheckOnFrameEncoded(true, vp8_info, tl_config));
    EXPECT_EQ(expected_temporal_idx[i], tl_config.packetizer_temporal_idx);
    EXPECT_EQ(expected_temporal_idx[i], tl_config.encoder_layer_id);
  ...
  EXPECT_TRUE(checker.CheckOnFrameEncoded(false, vp8_info, tl_config));
