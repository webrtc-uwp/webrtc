...
void VP8EncoderImpl::SetupTemporalLayers(int num_streams,
  ...
  if (num_streams == 1) {
    ...
    for (int i = 0; i < num_streams; ++i) {
      ...
      temporal_layers_checkers_.emplace_back(
          tl_factory->CreateChecker(i, num_temporal_layers, tl0_pic_idx_[0]));
    }
  }
}
...
void VP8EncoderImpl::PopulateCodecSpecific(
  ...
  temporal_layers_[stream_idx]->PopulateCodecSpecific(
      ...
      timestamp);
  RTC_DCHECK(temporal_layers_checkers_[stream_idx]->CheckOnFrameEncoded(
      (pkt.data.frame.flags & VPX_FRAME_IS_KEY) != 0, *vp8Info, tl_config));
  // Prepare next.
