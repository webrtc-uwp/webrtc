...
webrtc::TemporalLayersChecker* ScreenshareTemporalLayersFactory::CreateChecker(
    int simulcast_id,
    int temporal_layers,
    uint8_t initial_tl0_pic_idx) const {
  ...
  if (simulcast_id == 0) {
    ...
    TemporalLayersFactory rt_tl_factory;
    tlc = rt_tl_factory.CreateChecker(simulcast_id, temporal_layers,
                                      initial_tl0_pic_idx);
  }
  return tlc;
}
...
bool ScreenshareTemporalLayersChecker::CheckAndUpdateBufferState(
    BufferState* state,
    bool* need_sync,
    bool frame_is_keyframe,
