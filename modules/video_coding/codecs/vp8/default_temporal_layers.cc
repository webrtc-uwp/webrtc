...
TemporalLayersChecker* TemporalLayersFactory::CreateChecker(
    int /*simulcast_id*/,
    int temporal_layers,
    uint8_t initial_tl0_pic_idx) const {
...
// Values are sorted list of indecies in the pattern.
...
bool DefaultTemporalLayersChecker::CheckOnFrameEncoded(
  ...
  if (num_layers_ == 1) {
    expected_tl_idx = kNoTemporalIdx;
  } else if (frame_is_keyframe) {
    expected_tl_idx = 0;
  }
  if (codec_specific.temporalIdx != expected_tl_idx) {
    LOG(LS_ERROR) << "Frame has an incorrect temporal index.";
    fprintf(stderr,
            "Incorrect temporal index %d at pattern idx %d. Expected was %d\n",
            codec_specific.temporalIdx, pattern_idx_, expected_tl_idx);
    return false;
  }
  ...
  if (frame_config.last_buffer_flags &
    ...
    if (!last_.is_keyframe && referenced_layer > 0) {
      need_sync = false;
    }
  ...
  if (frame_config.arf_buffer_flags & TemporalLayers::BufferFlags::kReference) {
    ...
    if (!arf_.is_keyframe && referenced_layer > 0) {
      need_sync = false;
    }
    if (!arf_.is_keyframe) {
  ...
  if (frame_config.golden_buffer_flags &
    ...
    if (!golden_.is_keyframe && referenced_layer > 0) {
      need_sync = false;
    }
  ...
  if (frame_is_keyframe && num_layers_ > 1) {
    need_sync = true;
  }

  if (need_sync != codec_specific.layerSync) {
    LOG(LS_ERROR) << "Sync bit is set incorrectly on a frame.";
    fprintf(stderr, "sync is wrong: %d\n",
            static_cast<int>(codec_specific.layerSync));
    return false;
  }
  ...
  if (!frame_is_keyframe) {
    ...
    for (i = 0; i < dependencies.size(); ++i) {
      ...
      if (j == temporal_dependencies_[pattern_idx_].size() ||
        ...
        fprintf(stderr, "Illegal reference: from %d to %d\n", pattern_idx_,
                dependencies[i]);
        return false;
      }
    }
