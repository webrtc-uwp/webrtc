...
void void_main() {
  ...
  suppressor.Initialize(FLAGS_sample_rate_hz, detection_rate_hz,
                        FLAGS_num_channels);
  ...
  while (ReadBuffers(in_file, audio_buffer_size, FLAGS_num_channels,
    ...
    ASSERT_EQ(0, suppressor.Suppress(audio_buffer_f.get(), audio_buffer_size,
                                     FLAGS_num_channels, detection_buffer.get(),
    ...
    WritePCM(out_file, audio_buffer_size, FLAGS_num_channels,
             audio_buffer_f.get());
  }
