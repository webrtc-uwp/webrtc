...
namespace {
bool PacketIsLatest(const uint16_t* latest_seqno, uint16_t seqno) {
  if (!latest_seqno)
    return true;
...
bool RtpReceiverImpl::IncomingRtpPacket(const RTPHeader& rtp_header,
...

  {
    ...
    // the |recovered()| flag.
    if (PacketIsLatest(last_received_seqno_ ? &*last_received_seqno_ : nullptr,
                       rtp_header.sequenceNumber)) {
      last_received_seqno_.emplace(rtp_header.sequenceNumber);
