...
int32_t RTPPayloadRegistry::RegisterReceivePayload(const CodecInst& audio_codec,
  ...
  *created_new_payload = false;
  ...
  const auto insert_status = payload_type_map_.insert(
      std::make_pair(audio_codec.pltype, CreatePayloadType(audio_codec)));
...
int32_t RTPPayloadRegistry::RegisterReceivePayload(
  ...
  const auto insert_status = payload_type_map_.insert(
      std::make_pair(video_codec.plType, CreatePayloadType(video_codec)));
  RTC_DCHECK(insert_status.second);  // Insertion succeeded.
