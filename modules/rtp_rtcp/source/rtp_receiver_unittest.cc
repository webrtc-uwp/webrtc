...
TEST_F(RtpReceiverTest, RecordsInOrderTimestamps) {
  RTPHeader header;
  int64_t time1_ms = fake_clock_.TimeInMilliseconds();
  header.payloadType = kPcmuPayloadType;
  header.ssrc = kSsrc1;
...
TEST_F(RtpReceiverTest, RecordsTimestampAtSeqnoJump) {
  RTPHeader header;
  int64_t time1_ms = fake_clock_.TimeInMilliseconds();
  header.payloadType = kPcmuPayloadType;
  header.ssrc = kSsrc1;
  ...
  const int64_t receive_time_2 = fake_clock_.TimeInMilliseconds();
  const uint16_t seqno_2 = header.sequenceNumber;
  EXPECT_LT(static_cast<int16_t>(seqno_2 - seqno_1), 0);  // In the past.
