...
if (length(cout) > 3)
  load st.txt else disp('No cout file to load') end

      % initialize vector to zero index = find(st(1
                                                  : end, 1) == -1);
...
for
  i = 1 : functionOrder nonzeroelements = find(new (1 : end, i) > 0);
result(i, 1) = i;
...
% Compute each function's maximum complexity in encoding %
    and decoding respectively and then add it together : % result(i, 3) = (max(new (1
                                                                                    : end, i)) /
                                                                           0.03) /
    1000000;
result(i, 3) = (max(new (1
                         : size(new, 1) / 2, i)) /
                0.03) /
        1000000 +
    (max(new (size(new, 1) / 2 + 1
              : end, i)) /
     0.03) /
        1000000;
end
