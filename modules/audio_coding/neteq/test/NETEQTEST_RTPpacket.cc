...
int NETEQTEST_RTPpacket::setRTPheader(const webrtc::WebRtcRTPHeader* RTPinfo) {
  ...
  makeRTPheader(_datagram, RTPinfo->header.payloadType,
                RTPinfo->header.sequenceNumber, RTPinfo->header.timestamp,
                RTPinfo->header.ssrc, RTPinfo->header.markerBit);
...
uint16_t NETEQTEST_RTPpacket::parseRTPheader(webrtc::WebRtcRTPHeader* RTPinfo,
  ...
  if (payloadPtr) {
    *payloadPtr = reinterpret_cast<uint8_t*>(&rtp_data[i_startPosition >> 1]);
  }
...
void NETEQTEST_RTPpacket::parseBasicHeader(webrtc::WebRtcRTPHeader* RTPinfo,
  ...
  *i_CC = rtp_data[0] & 0xF;
  ...
  RTPinfo->header.timestamp = ((rtp_data[2] & 0xFF) << 24) |
                              ((rtp_data[2] & 0xFF00) << 8) |
                              (rtp_data[3] >> 8) | ((rtp_data[3] & 0xFF) << 8);
  RTPinfo->header.ssrc = ((rtp_data[4] & 0xFF) << 24) |
                         ((rtp_data[4] & 0xFF00) << 8) | (rtp_data[5] >> 8) |
                         ((rtp_data[5] & 0xFF) << 8);
}
...
int NETEQTEST_RTPpacket::calcHeaderLength(int i_X, int i_CC) const {
  ...
  if (i_X == 1) {
    ...
    if (_datagramLen > 2 * offset) {
      i_extlength =
          1 + (((rtp_data[offset]) >> 8) | ((rtp_data[offset] & 0xFF) << 8));
    }
  }
...
void NETEQTEST_RTPpacket::splitStereoSample(NETEQTEST_RTPpacket* slaveRtp,
                                            int stride) {
  if (!_payloadPtr || !slaveRtp || !slaveRtp->_payloadPtr || _payloadLen == 0 ||
      slaveRtp->_memSize < _memSize) {
    return;
  }
...
void NETEQTEST_RTPpacket::splitStereoFrame(NETEQTEST_RTPpacket* slaveRtp) {
  ...
  memmove(slaveRtp->_payloadPtr, _payloadPtr + _payloadLen / 2,
          _payloadLen / 2);
