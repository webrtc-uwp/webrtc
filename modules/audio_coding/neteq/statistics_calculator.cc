...
void StatisticsCalculator::ExpandedVoiceSamples(size_t num_samples) {
  ...
  size_t correction = std::min(num_samples, concealed_samples_correction_);
  concealed_samples_correction_ -= correction;
  lifetime_stats_.concealed_samples += num_samples - correction;
}
...
void StatisticsCalculator::ConcealedSamplesCorrection(int num_samples) {
  ...
  RTC_DCHECK_GE(num_samples, canceled_out);
