...
int RunTest(int argc, char* argv[]) {
  ...
  NetEqTest::DecoderMap codecs = {
      ...
      {FLAGS_avt_32, std::make_pair(NetEqDecoder::kDecoderAVT32kHz, "avt-32")},
      {FLAGS_avt_48, std::make_pair(NetEqDecoder::kDecoderAVT48kHz, "avt-48")},
  ...
  if (!FLAGS_replacement_audio_file.empty()) {
    ...
    std::set<uint8_t> forbidden_types =
        std_set_int32_to_uint8({FLAGS_g722, FLAGS_red, FLAGS_avt, FLAGS_avt_16,
                                FLAGS_avt_32, FLAGS_avt_48});
