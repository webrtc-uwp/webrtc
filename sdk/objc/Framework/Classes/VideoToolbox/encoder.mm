...
#include "sdk/objc/Framework/Classes/VideoToolbox/encoder.h"
...
inline CFDictionaryRef CreateCFDictionary(CFTypeRef* keys, CFTypeRef* values, size_t size) {
...
std::string CFStringToString(const CFStringRef cf_string) {
  ...
  size_t buffer_size =
      CFStringGetMaximumSizeForEncoding(CFStringGetLength(cf_string), kCFStringEncodingUTF8) + 1;
...
void SetVTSessionProperty(VTSessionRef session, CFStringRef key, int32_t value) {
  ...
  if (status != noErr) {
    ...
    LOG(LS_ERROR) << "VTSessionSetProperty failed to set: " << key_string << " to " << value << ": "
                  << status;
  }
}
...
void SetVTSessionProperty(VTSessionRef session, CFStringRef key, CFStringRef value) {
  ...
  if (status != noErr) {
    ...
    LOG(LS_ERROR) << "VTSessionSetProperty failed to set: " << key_string << " to " << val_string
                  << ": " << status;
  }
}
...
bool CopyVideoFrameToPixelBuffer(const rtc::scoped_refptr<webrtc::I420BufferInterface>& frame,
  ...
  int ret = libyuv::I420ToNV12(frame->DataY(),
                               frame->StrideY(),
                               frame->DataU(),
                               frame->StrideU(),
                               frame->DataV(),
...
void VTCompressionOutputCallback(void* encoder,
  ...
  std::unique_ptr<FrameEncodeParams> encode_params(reinterpret_cast<FrameEncodeParams*>(params));
  encode_params->encoder->OnEncodedFrame(status,
                                         info_flags,
                                         sample_buffer,
                                         encode_params->codec_specific_info,
                                         encode_params->width,
                                         encode_params->height,
                                         encode_params->render_time_ms,
                                         encode_params->timestamp,
...
int H264VideoToolboxEncoder::Encode(const VideoFrame& frame,
  ...
  CMTime presentation_time_stamp = CMTimeMake(frame.render_time_ms(), 1000);
  ...
  encode_params.reset(new internal::FrameEncodeParams(this,
                                                      codec_specific_info,
                                                      width_,
                                                      height_,
  ...
  encode_params->codec_specific_info.codecSpecific.H264.packetization_mode = packetization_mode_;
...
int H264VideoToolboxEncoder::SetChannelParameters(uint32_t packet_loss, int64_t rtt) {
  // Encoder doesn't know anything about packet loss or rtt so just return.
  return WEBRTC_VIDEO_CODEC_OK;
}

int H264VideoToolboxEncoder::SetRates(uint32_t new_bitrate_kbit, uint32_t frame_rate) {
...
void H264VideoToolboxEncoder::ConfigureCompressionSession() {
  ...
  internal::SetVTSessionProperty(compression_session_, kVTCompressionPropertyKey_RealTime, true);
  internal::SetVTSessionProperty(
      compression_session_, kVTCompressionPropertyKey_ProfileLevel, profile_);
  internal::SetVTSessionProperty(
      compression_session_, kVTCompressionPropertyKey_AllowFrameReordering, false);
  ...
  // 4 minutes).
  internal::SetVTSessionProperty(
      compression_session_, kVTCompressionPropertyKey_MaxKeyFrameInterval, 7200);
  internal::SetVTSessionProperty(
      compression_session_, kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration, 240);
}
...
void H264VideoToolboxEncoder::SetEncoderBitrateBps(uint32_t bitrate_bps) {
  if (compression_session_) {
    internal::SetVTSessionProperty(
        compression_session_, kVTCompressionPropertyKey_AverageBitRate, bitrate_bps);

    // TODO(tkchin): Add a helper method to set array value.
    int64_t data_limit_bytes_per_second_value =
        static_cast<int64_t>(bitrate_bps * internal::kLimitToAverageBitRateFactor / 8);
    CFNumberRef bytes_per_second = CFNumberCreate(
    ...
    CFArrayRef data_rate_limits = CFArrayCreate(nullptr, nums, 2, &kCFTypeArrayCallBacks);
    OSStatus status = VTSessionSetProperty(
        compression_session_, kVTCompressionPropertyKey_DataRateLimits, data_rate_limits);
    if (bytes_per_second) {
...
void H264VideoToolboxEncoder::OnEncodedFrame(OSStatus status,
                                             VTEncodeInfoFlags info_flags,
                                             CMSampleBufferRef sample_buffer,
  ...
  CFArrayRef attachments = CMSampleBufferGetSampleAttachmentsArray(sample_buffer, 0);
  ...
  std::unique_ptr<webrtc::RTPFragmentationHeader> header;
  {
    ...
    bool result =
        H264CMSampleBufferToAnnexBBuffer(sample_buffer, is_keyframe, buffer.get(), &header_raw);
...
VideoEncoder::ScalingSettings H264VideoToolboxEncoder::GetScalingSettings() const {
  return VideoEncoder::ScalingSettings(
      true, internal::kLowH264QpThreshold, internal::kHighH264QpThreshold);
}
}  // namespace webrtc
