...
- (void)removeAudioTrack:(RTCAudioTrack *)audioTrack {
  ...
  NSAssert(index != NSNotFound, @"|removeAudioTrack| called on unexpected RTCAudioTrack");
  if (index != NSNotFound && _nativeMediaStream->RemoveTrack(audioTrack.nativeAudioTrack)) {
...
- (void)removeVideoTrack:(RTCVideoTrack *)videoTrack {
  ...
  NSAssert(index != NSNotFound, @"|removeVideoTrack| called on unexpected RTCVideoTrack");
  if (index != NSNotFound && _nativeMediaStream->RemoveTrack(videoTrack.nativeVideoTrack)) {
...
- (instancetype)initWithNativeMediaStream:
  ...
  if (self = [super init]) {
    ...
    for (auto &track : audioTracks) {
      ...
      RTCAudioTrack *audioTrack = [[RTCAudioTrack alloc] initWithNativeTrack:track type:type];
      [_audioTracks addObject:audioTrack];
    }

    for (auto &track : videoTracks) {
      RTCMediaStreamTrackType type = RTCMediaStreamTrackTypeVideo;
      RTCVideoTrack *videoTrack = [[RTCVideoTrack alloc] initWithNativeTrack:track type:type];
      [_videoTracks addObject:videoTrack];
    }
  }
  return self;
