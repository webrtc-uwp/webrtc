...
RtpReceiverDelegateAdapter::RtpReceiverDelegateAdapter(RTCRtpReceiver *receiver) {
  RTC_CHECK(receiver);
  receiver_ = receiver;
}

void RtpReceiverDelegateAdapter::OnFirstPacketReceived(cricket::MediaType media_type) {
  RTCRtpMediaType packet_media_type = [RTCRtpReceiver mediaTypeForNativeMediaType:media_type];
...
- (RTCRtpParameters *)parameters {
  return [[RTCRtpParameters alloc] initWithNativeParameters:_nativeRtpReceiver->GetParameters()];
}

- (void)setParameters:(RTCRtpParameters *)parameters {
  if (!_nativeRtpReceiver->SetParameters(parameters.nativeParameters)) {
    RTCLogError(@"RTCRtpReceiver(%p): Failed to set parameters: %@", self, parameters);
  }
}
...
- (instancetype)initWithNativeRtpReceiver:
  ...
  if (self = [super init]) {
    ...
    RTCLogInfo(@"RTCRtpReceiver(%p): created receiver: %@", self, self.description);
