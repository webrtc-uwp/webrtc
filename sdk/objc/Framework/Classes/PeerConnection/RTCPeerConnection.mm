...
NSString *const kRTCPeerConnectionErrorDomain = @"org.webrtc.RTCPeerConnection";
...
class CreateSessionDescriptionObserverAdapter : public CreateSessionDescriptionObserver {
 public:
  CreateSessionDescriptionObserverAdapter(
      void (^completionHandler)(RTCSessionDescription *sessionDescription, NSError *error)) {
    completion_handler_ = completionHandler;
  }

  ~CreateSessionDescriptionObserverAdapter() { completion_handler_ = nil; }
  ...
  void OnSuccess(SessionDescriptionInterface *desc) override {
    ...
    RTCSessionDescription *session =
        [[RTCSessionDescription alloc] initWithNativeDescription:description.get()];
 ...
 private:
  void (^completion_handler_)(RTCSessionDescription *sessionDescription, NSError *error);
};

class SetSessionDescriptionObserverAdapter : public SetSessionDescriptionObserver {
 public:
  SetSessionDescriptionObserverAdapter(void (^completionHandler)(NSError *error)) {
    completion_handler_ = completionHandler;
  }

  ~SetSessionDescriptionObserverAdapter() { completion_handler_ = nil; }
...
PeerConnectionDelegateAdapter::PeerConnectionDelegateAdapter(RTCPeerConnection *peerConnection) {
...
void PeerConnectionDelegateAdapter::OnSignalingChange(
  ...
  RTCSignalingState state = [[RTCPeerConnection class] signalingStateForNativeState:new_state];
...
void PeerConnectionDelegateAdapter::OnDataChannel(
  ...
  RTCDataChannel *dataChannel = [[RTCDataChannel alloc] initWithNativeDataChannel:data_channel];
  RTCPeerConnection *peer_connection = peer_connection_;
...
void PeerConnectionDelegateAdapter::OnIceConnectionChange(
  ...
  RTCPeerConnection *peer_connection = peer_connection_;
  [peer_connection.delegate peerConnection:peer_connection didChangeIceConnectionState:state];
}
...
- (instancetype)initWithFactory:(RTCPeerConnectionFactory *)factory
  ...
  if (self = [super init]) {
    ...
    _peerConnection =
        factory.nativeFactory->CreatePeerConnection(*config, nullptr, nullptr, _observer.get());
    if (!_peerConnection) {
      return nil;
    }
...
- (RTCSessionDescription *)localDescription {
  ...
  return description ? [[RTCSessionDescription alloc] initWithNativeDescription:description] : nil;
}
...
- (void)offerForConstraints:(RTCMediaConstraints *)constraints
          completionHandler:(void (^)(RTCSessionDescription *sessionDescription,
                                      NSError *error))completionHandler {
  rtc::scoped_refptr<webrtc::CreateSessionDescriptionObserverAdapter> observer(
      new rtc::RefCountedObject<webrtc::CreateSessionDescriptionObserverAdapter>(
          completionHandler));
...
- (void)setLocalDescription:(RTCSessionDescription *)sdp
  ...
  rtc::scoped_refptr<webrtc::SetSessionDescriptionObserverAdapter> observer(
      new rtc::RefCountedObject<webrtc::SetSessionDescriptionObserverAdapter>(completionHandler));
...
- (BOOL)startRtcEventLogWithFilePath:(NSString *)filePath maxSizeInBytes:(int64_t)maxSizeInBytes {
  ...
  _hasStartedRtcEventLog = _peerConnection->StartRtcEventLog(fd, maxSizeInBytes);
