      ...
      switch (result) {
        ...
        default:
          ...
          if (decodeTimeMs > MAX_DECODE_TIME_MS) {
            Logging.e(TAG,
                "Very high decode time: " + decodeTimeMs + "ms"
  ...
  private DecodedTextureBuffer dequeueTextureBuffer(int dequeueTimeoutMs) {
    ...
    if ((dequeuedSurfaceOutputBuffers.size()
      ...
      if (dequeueTimeoutMs > 0) {
        ...
        Logging.w(TAG,
            "Draining decoder. Dropping frame with TS: " + droppedFrame.presentationTimeStampMs
                + ". Total number of dropped frames: " + droppedFrames);
      } else {
        Logging.w(TAG,
            "Too many output buffers " + dequeuedSurfaceOutputBuffers.size()
