...
#include "sdk/android/src/jni/androidnetworkmonitor_jni.h"
// Adding 'nogncheck' to disable the gn include headers check.
// We don't want to always depend on audio and video related targets.
#include "sdk/android/src/jni/androidvideotracksource.h"  // nogncheck
#include "sdk/android/src/jni/audio_jni.h"
#include "sdk/android/src/jni/classreferenceholder.h"
#include "sdk/android/src/jni/jni_helpers.h"
#include "sdk/android/src/jni/media_jni.h"
#include "sdk/android/src/jni/ownedfactoryandthreads.h"
#include "sdk/android/src/jni/rtcstatscollectorcallbackwrapper.h"
#include "sdk/android/src/jni/video_jni.h"
...
#include "voice_engine/include/voe_base.h"  // nogncheck
...
static DataChannelInit JavaDataChannelInitToNative(JNIEnv* jni,
                                                   jobject j_init) {
...
class PCOJava : public PeerConnectionObserver {
 public:
  ...
  void OnIceGatheringChange(
    ...
    ScopedLocalRefFrame local_ref_frame(jni());
    jmethodID m =
        GetMethodID(jni(), *j_observer_class_, "onIceGatheringChange",
                    "(Lorg/webrtc/PeerConnection$IceGatheringState;)V");
    jobject new_state_enum =
        JavaEnumFromIndex(jni(), "PeerConnection$IceGatheringState", new_state);
    jni()->CallVoidMethod(*j_observer_global_, m, new_state_enum);
    CHECK_EXCEPTION(jni()) << "error during CallVoidMethod";
  }
  ...
  void OnAddStream(rtc::scoped_refptr<MediaStreamInterface> stream) override {
    ...
    for (const auto& track : stream->GetAudioTracks()) {
      ...
      CHECK_EXCEPTION(jni()) << "error during NewObject";
      jfieldID audio_tracks_id =
          GetFieldID(jni(), *j_media_stream_class_, "audioTracks",
    ...
    for (const auto& track : stream->GetVideoTracks()) {
      ...
      jobject j_track =
          jni()->NewObject(*j_video_track_class_, j_video_track_ctor_,
                           reinterpret_cast<jlong>(track.get()), id);
      CHECK_EXCEPTION(jni()) << "error during NewObject";
      jfieldID video_tracks_id =
          GetFieldID(jni(), *j_media_stream_class_, "videoTracks",
                     "Ljava/util/LinkedList;");
...
static jobject JavaSdpFromNativeSdp(JNIEnv* jni,
...
class CreateSdpObserverWrapper
 ...
 public:
  CreateSdpObserverWrapper(JNIEnv* jni,
                           jobject j_observer,
                           ConstraintsWrapper* constraints)
...
class SetSdpObserverWrapper
 ...
 public:
  SetSdpObserverWrapper(JNIEnv* jni,
                        jobject j_observer,
                        ConstraintsWrapper* constraints)
      : SdpObserverWrapper(jni, j_observer, constraints) {}
...
JOW(jobject, DataChannel_state)(JNIEnv* jni, jobject j_dc) {
  return JavaEnumFromIndex(jni, "DataChannel$State",
...
static PeerConnectionInterface::BundlePolicy JavaBundlePolicyToNativeType(
    JNIEnv* jni,
...
static PeerConnectionInterface::RtcpMuxPolicy JavaRtcpMuxPolicyToNativeType(
    JNIEnv* jni,
...
JavaTcpCandidatePolicyToNativeType(JNIEnv* jni,
...
JavaContinualGatheringPolicyToNativeType(JNIEnv* jni,
                                         jobject j_gathering_policy) {
  std::string enum_name =
...
static void JavaRTCConfigurationToJsepRTCConfiguration(
  ...
  rtc_config->continual_gathering_policy =
      JavaContinualGatheringPolicyToNativeType(jni,
...
(JNIEnv* jni, jobject j_pc, jobject j_observer, jobject j_constraints) {
  ConstraintsWrapper* constraints = new ConstraintsWrapper(jni, j_constraints);
  rtc::scoped_refptr<CreateSdpObserverWrapper> observer(
      new rtc::RefCountedObject<CreateSdpObserverWrapper>(jni, j_observer,
                                                          constraints));
...
static SessionDescriptionInterface* JavaSdpToNativeSdp(JNIEnv* jni,
                                                       jobject j_sdp) {
