...
TEST(PayloadRouterTest, SetRtpPayloadStates) {
  NiceMock<MockRtpRtcp> rtp1;
  NiceMock<MockRtpRtcp> rtp2;
  std::vector<RtpRtcp*> modules = {&rtp1, &rtp2};
  PayloadRouter payload_router(modules, {kSsrc1, kSsrc2}, kPayloadType);
  ...
  payload_router.SetRtpPayloadStates({{kSsrc1, state1}, {kSsrc2, state2}});

  std::map<uint32_t, RtpPayloadState> states =
      payload_router.GetRtpPayloadStates();
  EXPECT_EQ(2u, states.size());
  EXPECT_EQ(kInitialPictureId1, states[kSsrc1].picture_id);
  EXPECT_EQ(kInitialPictureId2, states[kSsrc2].picture_id);

  // Setting unknown ssrc has no effect.
  ++state1.picture_id;
  ++state2.picture_id;
  payload_router.SetRtpPayloadStates({{kSsrc1 + 1, state1}, {kSsrc2, state2}});
  states = payload_router.GetRtpPayloadStates();
  EXPECT_EQ(2u, states.size());
  EXPECT_EQ(kInitialPictureId1 + 0, states[kSsrc1].picture_id);
...
TEST_F(TestWithForcedFallbackEnabled, PictureIdWraps) {
  ...
  payload_router.SetRtpPayloadStates({{kSsrc1, state1}});
  ...
  // State should hold next picture id to set.
  std::map<uint32_t, RtpPayloadState> states =
      payload_router.GetRtpPayloadStates();
