...
class PayloadRouter::RtpPayloadParams final {
 public:
  explicit RtpPayloadParams(const uint32_t ssrc, const RtpPayloadState* state)
      : ssrc_(ssrc) {
...
PayloadRouter::PayloadRouter(const std::vector<RtpRtcp*>& rtp_modules,
  ...
  for (uint32_t ssrc : ssrcs) {
    // Restore state if previous existed.
    ...
    params_.push_back(
        std::unique_ptr<RtpPayloadParams>(new RtpPayloadParams(ssrc, state)));
  }
}
