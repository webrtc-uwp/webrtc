...
class PictureIdObserver : public test::RtpRtcpObserver {
 public:
  ...
  void ResetPictureIds() {
    rtc::CritScope lock(&crit_);
    last_observed_picture_id_.clear();
  }
...
class PictureIDTest : public test::CallTest {
 public:
  ...
  void TestPictureIdContinuousAfterReconfigure(std::vector<int> ssrc_counts);
  void TestPictureIdIncreaseAfterRecreateStreams(std::vector<int> ssrc_counts);
...
TEST_F(PictureIDTest, PictureIdContinuousAfterReconfigureVp8) {
  ...
  //  std::vector<int> ssrc_counts = {1, 3, 3, 1, 1};
