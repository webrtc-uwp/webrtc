...
void ReportBlockStats::Store(const RtcpStatistics& rtcp_stats,
  ...
  block.cumulative_packets_lost = rtcp_stats.cumulative_packets_lost;
  ...
  block.interarrival_jitter = rtcp_stats.interarrival_jitter;
...
RTCPReportBlock ReportBlockStats::AggregateAndStore(
  ...
  for (; report_block != report_blocks.end(); ++report_block) {
    aggregate.cumulative_packets_lost += report_block->cumulative_packets_lost;
    aggregate.interarrival_jitter += report_block->interarrival_jitter;
  ...
  aggregate.interarrival_jitter = static_cast<uint32_t>(
      (aggregate.interarrival_jitter + report_blocks.size() / 2) /
      report_blocks.size());
  return aggregate;
}
...
void ReportBlockStats::StoreAndAddPacketIncrement(
  ...
  if (prev_report_block != prev_report_blocks_.end()) {
    ...
    int cum_loss_diff = report_block.cumulative_packets_lost -
                        prev_report_block->second.cumulative_packets_lost;
