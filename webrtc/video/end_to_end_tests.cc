    ...
    bool CheckReceiveStats() {
      for (size_t i = 0; i < receive_streams_.size(); ++i) {
        ...
        receive_stats_filled_["StatisticsUpdated"] |=
            stats.rtcp_stats.cumulative_packets_lost != 0 ||
            stats.rtcp_stats.extended_highest_sequence_number != 0 ||
            stats.rtcp_stats.fraction_lost != 0 ||
            stats.rtcp_stats.interarrival_jitter != 0;
    ...
    bool CheckSendStats() {
      ...
      for (std::map<uint32_t, VideoSendStream::StreamStats>::const_iterator it =
        ...
        send_stats_filled_[CompoundKey("StatisticsUpdated", it->first)] |=
            stream_stats.rtcp_stats.cumulative_packets_lost != 0 ||
