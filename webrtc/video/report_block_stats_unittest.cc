...
class ReportBlockStatsTest : public ::testing::Test {
 protected:
  ...
  RtcpStatistics RtcpReportBlockToRtcpStatistics(
    ...
    block.cumulative_packets_lost = stats.cumulative_packets_lost;
    block.fraction_lost = stats.fraction_lost;
    block.extended_highest_sequence_number =
        stats.extended_highest_sequence_number;
    block.interarrival_jitter = stats.interarrival_jitter;
    return block;
  }
...
TEST_F(ReportBlockStatsTest, AggregateAndStore_NoSsrc) {
  ...
  EXPECT_EQ(0U, aggregated.cumulative_packets_lost);
  EXPECT_EQ(0U, aggregated.interarrival_jitter);
...
TEST_F(ReportBlockStatsTest, AggregateAndStore_OneSsrc) {
  ...
  EXPECT_EQ(10U, aggregated.cumulative_packets_lost);
  EXPECT_EQ(777U, aggregated.interarrival_jitter);
  EXPECT_EQ(24000U, aggregated.extended_highest_sequence_number);

  aggregated = stats.AggregateAndStore(ssrc1block2_);
  EXPECT_EQ(0U, aggregated.fraction_lost);
  EXPECT_EQ(15U, aggregated.cumulative_packets_lost);
  EXPECT_EQ(222U, aggregated.interarrival_jitter);
...
TEST_F(ReportBlockStatsTest, AggregateAndStore_TwoSsrcs) {
  ...
  EXPECT_EQ(0U, aggregated.fraction_lost);
  EXPECT_EQ(10U + 111U, aggregated.cumulative_packets_lost);
  EXPECT_EQ((777U + 555U) / 2, aggregated.interarrival_jitter);
  EXPECT_EQ(0U, aggregated.extended_highest_sequence_number);
  ...
  EXPECT_EQ(15U + 136U, aggregated.cumulative_packets_lost);
  EXPECT_EQ((222U + 888U) / 2, aggregated.interarrival_jitter);
  EXPECT_EQ(0U, aggregated.extended_highest_sequence_number);
