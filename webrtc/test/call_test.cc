...
void CallTest::RunBaseTest(BaseTest* test) {
  ...
  if (num_audio_streams_ > 0) {
    ...
    send_config.audio_state =
        AudioState::Create(audio_state_config, apm_send_.get());
  }
  ...
  if (test->ShouldCreateReceivers()) {
    ...
    if (num_audio_streams_ > 0) {
      ...
      audio_state_config.audio_mixer = AudioMixerImpl::Create();
      recv_config.audio_state =
          AudioState::Create(audio_state_config, apm_recv_.get());
    }
...
void CallTest::CreateVoiceEngines() {
  apm_send_.reset(AudioProcessing::Create());
  ...
  apm_recv_.reset(AudioProcessing::Create());
