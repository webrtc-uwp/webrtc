...
class RtpDemuxer {
 ...
 private:
  ...
  // A sink should only be associated with an MID if the sink is able to handle
  // all RTP packets with the MID. In particular, if a stream cannot be uniquely
  // identified by MID (e.g., multiple encodings in the same track), the client
  // will need to do further demuxing.
  // TODO(steveanton): Add demuxing RTP to sinks based on both MID and RSID.
