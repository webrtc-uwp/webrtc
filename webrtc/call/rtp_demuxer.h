...
class RtpDemuxer {
 ...
 private:
  ...
  // as their criteria.
  // Payload Type mapping is a multimap because it's possible for different
  // criteria to share attributes. If multiple sinks are associated with a
  // value, the demuxing algorithm will not use that attribute alone to choose
  // the receiving sink.
