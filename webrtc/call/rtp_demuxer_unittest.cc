...
// complete in AddSink (see note left there).
...
TEST_F(RtpDemuxerTest, NoCallbackOnMidSinkRemovedAfterFirstPacket) {
  ...
  EXPECT_TRUE(demuxer_.OnRtpPacket(*p1));
...
TEST_F(RtpDemuxerTest, FirstSsrcAssociatedWithAnRsidIsNotForgotten) {
  ...
  EXPECT_CALL(sink_a, OnRtpPacket(SamePacketAs(*packet_b))).Times(0);
  ...
  EXPECT_CALL(sink_a, OnRtpPacket(SamePacketAs(*packet_c))).Times(0);
  EXPECT_CALL(sink_b, OnRtpPacket(SamePacketAs(*packet_c))).Times(0);
  EXPECT_CALL(sink_c, OnRtpPacket(SamePacketAs(*packet_c))).Times(0);
...
TEST_F(RtpDemuxerTest,
  ...
  EXPECT_CALL(ssrc_sink, OnRtpPacket(SamePacketAs(*packet))).Times(0);
...
TEST_F(RtpDemuxerTest, DontLearnMidSsrcBindingBeforeSinkAdded) {
  ...
  auto p1 = CreatePacketWithSsrcMid(ssrc, mid);
  EXPECT_FALSE(demuxer_.OnRtpPacket(*p1));
...
TEST_F(RtpDemuxerTest, DontForgetMidSsrcBindingWhenSinkRemoved) {
  ...
  EXPECT_CALL(sink1, OnRtpPacket(SamePacketAs(*packet_with_mid)));
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_with_mid));
...
TEST_F(RtpDemuxerTest, LearnMidThenRsidSeparatelyAndRouteBySsrc) {
  ...
  auto packet_with_mid = CreatePacketWithSsrcMid(ssrc, mid);
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_mid))).Times(0);
  EXPECT_FALSE(demuxer_.OnRtpPacket(*packet_with_mid));

  auto packet_with_rsid = CreatePacketWithSsrcRsid(ssrc, rsid);
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_rsid))).Times(1);
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_with_rsid));
...
TEST_F(RtpDemuxerTest, LearnRsidThenMidSeparatelyAndRouteBySsrc) {
  ...
  auto packet_with_rsid = CreatePacketWithSsrcRsid(ssrc, rsid);
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_rsid))).Times(0);
  EXPECT_FALSE(demuxer_.OnRtpPacket(*packet_with_rsid));

  auto packet_with_mid = CreatePacketWithSsrcMid(ssrc, mid);
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_mid))).Times(1);
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_with_mid));
...
TEST_F(RtpDemuxerTest, DontLearnMidRsidBindingBeforeSinkAdded) {
  ...
  EXPECT_FALSE(demuxer_.OnRtpPacket(*packet_with_both));

  MockRtpPacketSink sink;
  AddSinkBothMidRsid(mid, rsid, &sink);

  auto packet_with_ssrc = CreatePacketWithSsrc(ssrc);
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_ssrc))).Times(0);
...
TEST_F(RtpDemuxerTest, DontForgetMidRsidBindingWhenSinkRemoved) {
  ...
  auto packet_with_both = CreatePacketWithSsrcMidRsid(ssrc, mid, rsid);
  EXPECT_CALL(sink1, OnRtpPacket(SamePacketAs(*packet_with_both)));
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_with_both));
...
TEST_F(RtpDemuxerTest, LearnMidRsidBindingAfterSinkAdded) {
  ...
  auto packet_with_both = CreatePacketWithSsrcMidRsid(ssrc, mid, rsid);
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_both)));
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_with_both));
...
TEST_F(RtpDemuxerTest, RoutedByPayloadTypeLatchesSsrc) {
  ...
  EXPECT_CALL(sink, OnRtpPacket(SamePacketAs(*packet_with_pt))).Times(1);
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_with_pt));
...
TEST_F(RtpDemuxerTest, PacketWithSameRsidDifferentMidRoutedToProperSink) {
  ...
  EXPECT_TRUE(demuxer_.OnRtpPacket(*packet_mid1));
...
TEST_F(RtpDemuxerTest, MaliciousPeerCannotCauseMemoryOveruse) {
  constexpr int kTooManySsrcBindings = 1001;
  const std::string mid = "v";
  ...
  EXPECT_CALL(observer, OnSsrcBoundToMid(_, _))
      .Times(AtMost(kTooManySsrcBindings - 1));

  for (int i = 0; i < kTooManySsrcBindings + 1; i++) {
