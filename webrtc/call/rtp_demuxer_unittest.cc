...
TEST(RtpDemuxerTest, OnRtpPacketCalledOnCorrectSinkByMid) {
  ...
  for (size_t i = 0; i < arraysize(mids); i++) {
    demuxer.AddMidSink(mids[i], &sinks[i]);
  }
...
TEST(RtpDemuxerTest, RsidResolutionObserversNotifiedEachTime) {
  RtpDemuxer demuxer;
  ...
  MockRsidResolutionObserver rsid_resolution_observer;

  demuxer.RegisterRsidResolutionObserver(&rsid_resolution_observer);

  EXPECT_CALL(rsid_resolution_observer, OnRsidResolved(rsid, ssrc)).Times(2);
  ...
  // Test tear-down.
  demuxer.RemoveSink(&sink);
  demuxer.DeregisterRsidResolutionObserver(&rsid_resolution_observer);
}
...
TEST(RtpDemuxerTest, RoutedByMidWhenSsrcChanges) {
  RtpDemuxer demuxer;
