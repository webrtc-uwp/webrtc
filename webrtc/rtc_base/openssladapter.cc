...
int OpenSSLAdapter::BeginSSL() {
  ...
  // Enable session caching, if configured.
  if (factory_) {
...
int OpenSSLAdapter::NewSSLSessionCallback(SSL* ssl, SSL_SESSION* session) {
  ...
  return 1;  // We've taken ownership of the session, don't free it.
}
...
OpenSSLAdapter* OpenSSLAdapterFactory::CreateAdapter(AsyncSocket* socket) {
  if (!ssl_ctx_) {
    ssl_ctx_ = OpenSSLAdapter::CreateContext(ssl_mode_, true);
    if (!ssl_ctx_) {
      return nullptr;
    }
  }
