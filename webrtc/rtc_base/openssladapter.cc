...
int OpenSSLAdapter::BeginSSL() {
  LOG(LS_INFO) << "Starting TLS connection to " << ssl_host_name_;
    ...
    if (factory_) {
      ...
      if (cached) {
        if (SSL_set_session(ssl_, cached) == 0) {
          LOG(LS_WARNING) << "Failed to apply TLS session from cache";
          err = -1;
          goto ssl_error;
        }

        LOG(LS_INFO) << "Attempting to resume TLS session to "
                     << ssl_host_name_;
      }
    }
  }
...
int OpenSSLAdapter::ContinueSSL() {
  ...
  case SSL_ERROR_NONE:
    ...
    LOG(LS_INFO) << "TLS connection to " << ssl_host_name_
                 << (IsResumedSession() ? " resumed" : " established");
...
int OpenSSLAdapter::NewSSLSessionCallback(SSL* ssl, SSL_SESSION* session) {
  ...
  LOG(LS_INFO) << "Caching TLS session for " << stream->ssl_host_name_;
