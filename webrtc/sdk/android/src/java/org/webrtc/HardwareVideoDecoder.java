...
class HardwareVideoDecoder implements VideoDecoder {
  ...
  // Width & height are read on the output thread and may be changed by the decode thread.
  ...
  private void deliverDecodedFrame() {
    ...
    try {
      ...
      // Possible optimization:  avoid copying data here.  Wrap the output buffers into the frame
      // buffer without copying or reformatting.
      // Possible optimization:  use libyuv via JNI to copy/reformatting data.
      ...
      // TODO(mellem):  Add decodeTimeMs.
      // Note that qp is parsed on the C++ side.
      callback.onDecodedFrame(frame, null /* decodeTimeMs */, null /* qp */);
      frame.release();
  ...
  // Possible optimization:  do this using libyuv via JNI.
  ...
  private void checkOnOutputThread() {
    if (outputThread.getId() != Thread.currentThread().getId()) {
      throw new IllegalStateException("Method expected to run on output thread " + outputThread
          + " but was called on " + Thread.currentThread());
    }
  }

  private void checkOnDecoderThread() {
    if (decoderThread.getId() != Thread.currentThread().getId()) {
      throw new IllegalStateException("Method expected to run on decoder thread " + decoderThread
          + " but was called on " + Thread.currentThread());
    }
  }
}
