...
class MediaCodecVideoEncoder : public webrtc::VideoEncoder {
 ...
 private:
  ...
  std::vector<uint8_t> scale_buffer_;  // Temporary buffer used for scaling.
...
bool MediaCodecVideoEncoder::IsTextureFrame(JNIEnv* jni,
  ...
  AndroidVideoFrameBuffer::AndroidType android_type =
      android_buffer->android_type();
  if (android_type == AndroidVideoFrameBuffer::AndroidType::kTextureBuffer) {
    return true;
  }

  if (android_type == AndroidVideoFrameBuffer::AndroidType::kJavaBuffer) {
    jobject java_buffer =
        static_cast<AndroidVideoBuffer*>(android_buffer)->video_frame_buffer();
    return jni->IsInstanceOf(java_buffer, *j_video_frame_texture_buffer_class_);
  }
...
bool MediaCodecVideoEncoder::EncodeByteBuffer(JNIEnv* jni,
  ...
  if (!FillInputBuffer(jni, input_buffer_index, i420_buffer->DataY(),
                       ...
                       i420_buffer->StrideV(), 0, 0, i420_buffer->width(),
                       i420_buffer->height())) {
    return false;
  }
...
bool MediaCodecVideoEncoder::FillInputBuffer(JNIEnv* jni,
  ...
  buffer_y += crop_x + crop_y * stride_y;
  buffer_u += crop_x / 2 + crop_y / 2 * stride_u;
  buffer_v += crop_x / 2 + crop_y / 2 * stride_v;

  uint8_t const* scaled_buffer_y;
  uint8_t const* scaled_buffer_u;
  uint8_t const* scaled_buffer_v;
  int scaled_stride_y;
  int scaled_stride_u;
  int scaled_stride_v;

  if (width_ != crop_width || height_ != crop_height) {
    scaled_stride_y = width_;
    scaled_stride_u = (width_ + 1) / 2;
    scaled_stride_v = (width_ + 1) / 2;

    int chroma_height = (height_ + 1) / 2;

    int scaled_size_y = height_ * scaled_stride_y;
    int scaled_size_u = chroma_height * scaled_stride_u;
    int scaled_size_v = chroma_height * scaled_stride_v;

    scale_buffer_.resize(scaled_size_y + scaled_size_u + scaled_size_v);

    uint8_t* mutable_scaled_buffer_y = &scale_buffer_[0];
    uint8_t* mutable_scaled_buffer_u = &scale_buffer_[scaled_size_y];
    uint8_t* mutable_scaled_buffer_v =
        &scale_buffer_[scaled_size_y + scaled_size_u];

    RTC_CHECK(!libyuv::I420Scale(
        buffer_y, stride_y, buffer_u, stride_u, buffer_v, stride_v, crop_width,
        crop_height, mutable_scaled_buffer_y, scaled_stride_y,
        mutable_scaled_buffer_u, scaled_stride_u, mutable_scaled_buffer_v,
        scaled_stride_v, width_, height_, libyuv::kFilterBox))
        << "I420Scale failed";

    scaled_buffer_y = mutable_scaled_buffer_y;
    scaled_buffer_u = mutable_scaled_buffer_u;
    scaled_buffer_v = mutable_scaled_buffer_v;
  } else {
    scaled_buffer_y = buffer_y;
    scaled_buffer_u = buffer_u;
    scaled_buffer_v = buffer_v;
    scaled_stride_y = stride_y;
    scaled_stride_u = stride_u;
    scaled_stride_v = stride_v;
  }

  RTC_CHECK(!libyuv::ConvertFromI420(
      scaled_buffer_y, scaled_stride_y, scaled_buffer_u, scaled_stride_u,
      scaled_buffer_v, scaled_stride_v, yuv_buffer, width_, width_, height_,
      encoder_fourcc_))
 ...
 jint crop_height) {
  ...
  reinterpret_cast<MediaCodecVideoEncoder*>(native_encoder)
      ->FillInputBuffer(jni, input_buffer, buffer_y, stride_y, buffer_u,
                        stride_u, buffer_v, stride_v, crop_x, crop_y,
