...
static void JavaIceServersToJsepIceServers(
  ...
  for (jobject j_ice_server : Iterable(jni, j_ice_servers)) {
    ...
    jfieldID j_ice_server_tls_host_name_id = GetFieldID(
        jni, j_ice_server_class, "tlsHostName", "Ljava/lang/String;");
    ...
    jstring tls_host_name = reinterpret_cast<jstring>(
        GetObjectField(jni, j_ice_server, j_ice_server_tls_host_name_id));
    PeerConnectionInterface::IceServer server;
    server.uri = JavaToStdString(jni, uri);
    server.username = JavaToStdString(jni, username);
    server.password = JavaToStdString(jni, password);
    server.tls_cert_policy = tls_cert_policy;
    server.tls_host_name = JavaToStdString(jni, tls_host_name);
