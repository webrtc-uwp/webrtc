...
int32_t VideoDecoderWrapper::InitDecode(
  ...
  return HandleReturnCode(jni, ret, true /* try_reset */);
}
...
int32_t VideoDecoderWrapper::Release() {
  ...
  initialized_ = false;
  return HandleReturnCode(jni, ret, false /* try_reset */);
}
...
int32_t VideoDecoderWrapper::HandleReturnCode(JNIEnv* jni,
  ...
  if (value < 0) {  // Any errors are represented by negative values.
    if (try_reset && Reset()) {
      // Reset was successful, do not fallback.
      return WEBRTC_VIDEO_CODEC_OK;
    } else {
      if (initialized_) {
        Release();
...
bool VideoDecoderWrapper::Reset() {
  if (reset_tries_ >= kMaxResetTries) {
    return false;
  }
  reset_tries_++;
  LOG(LS_WARNING) << "Resetting decoder, try number: " << reset_tries_;
  if (Release() != WEBRTC_VIDEO_CODEC_OK) {
    return false;
  }
  return InitDecode(&codec_settings_, number_of_cores_) ==
