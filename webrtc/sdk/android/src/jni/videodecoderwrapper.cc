...
VideoDecoderWrapper::VideoDecoderWrapper(JNIEnv* jni, jobject decoder)
  ...
  video_frame_get_timestamp_rtp_method_ =
      jni->GetMethodID(*video_frame_class_, "getTimestampRtp", "()J");
...
void VideoDecoderWrapper::OnDecodedFrame(JNIEnv* jni,
                                         ...
                                         jobject jqp) {
  VideoFrame frame(I420Buffer::Create(640, 480), (VideoRotation)0, 0);
  frame.set_timestamp(
      jni->CallLongMethod(jframe, video_frame_get_timestamp_rtp_method_));
...
(JNIEnv* jni,
 ...
 jobject jdecode_time_ms,
 jobject jqp) {
  LOG(LS_INFO) << "VideoDecoderWrapperCallback_nativeOnDecodedFrame";
