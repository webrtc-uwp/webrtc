...
Matrix Matrix::fromAndroidGraphicsMatrix(JNIEnv* jni, jobject j_matrix) {
  ...
  matrix.elem_[3 + 3 * 3] = 1;  // Bottom-left corner should be 1.
  return matrix;
}
...
AndroidTextureBuffer::AndroidTextureBuffer(
    ...
    : AndroidVideoFrameBuffer(AndroidType::kTextureBuffer),
      width_(width),
      height_(height),
...
jobject AndroidVideoBuffer::ToJavaI420Frame(JNIEnv* jni,
  ...
  jmethodID j_i420_frame_ctor_id = GetMethodID(
      jni, j_i420_frame_class, "<init>", "(III[I[Ljava/nio/ByteBuffer;J)V");

  jclass j_buffer_class = FindClass(jni, "org/webrtc/VideoFrame$Buffer");
  jmethodID to_i420_id = jni->GetMethodID(
      j_buffer_class, "toI420", "()Lorg/webrtc/VideoFrame$I420Buffer;");

  jclass j_i420_buffer_class =
      FindClass(jni, "org/webrtc/VideoFrame$I420Buffer");
  jmethodID j_get_data_y_id = jni->GetMethodID(j_i420_buffer_class, "getDataY",
                                               "()Ljava/nio/ByteBuffer;");
  jmethodID j_get_data_u_id = jni->GetMethodID(j_i420_buffer_class, "getDataU",
                                               "()Ljava/nio/ByteBuffer;");
  jmethodID j_get_data_v_id = jni->GetMethodID(j_i420_buffer_class, "getDataV",
                                               "()Ljava/nio/ByteBuffer;");
  jmethodID j_get_stride_y_id =
      jni->GetMethodID(j_i420_buffer_class, "getStrideY", "()I");
  jmethodID j_get_stride_u_id =
      jni->GetMethodID(j_i420_buffer_class, "getStrideU", "()I");
  jmethodID j_get_stride_v_id =
      jni->GetMethodID(j_i420_buffer_class, "getStrideV", "()I");

  // TODO(sakal): Special handling for texture frames.
  jobject j_i420_buffer =
      jni->CallObjectMethod(*j_video_frame_buffer_, to_i420_id);
  jintArray strides = jni->NewIntArray(3);
  jint* strides_array = jni->GetIntArrayElements(strides, NULL);
  strides_array[0] = jni->CallIntMethod(j_i420_buffer, j_get_stride_y_id);
  strides_array[1] = jni->CallIntMethod(j_i420_buffer, j_get_stride_u_id);
  strides_array[2] = jni->CallIntMethod(j_i420_buffer, j_get_stride_v_id);
  jni->ReleaseIntArrayElements(strides, strides_array, 0);
  jobjectArray planes = jni->NewObjectArray(3, j_byte_buffer_class, NULL);
  jobject y_buffer = jni->CallObjectMethod(j_i420_buffer, j_get_data_y_id);
  jobject u_buffer = jni->CallObjectMethod(j_i420_buffer, j_get_data_u_id);
  jobject v_buffer = jni->CallObjectMethod(j_i420_buffer, j_get_data_v_id);
  jni->SetObjectArrayElement(planes, 0, y_buffer);
  jni->SetObjectArrayElement(planes, 1, u_buffer);
  jni->SetObjectArrayElement(planes, 2, v_buffer);
  // Java code just uses the native frame to hold a reference to ther buffer so
  // this is okay.
...
webrtc::VideoFrame AndroidVideoBufferFactory::CreateFrame(
  ...
  return webrtc::VideoFrame(buffer, timestamp_rtp, timestamp_ns / 1000 / 1000,
