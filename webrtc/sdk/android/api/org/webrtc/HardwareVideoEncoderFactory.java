...
public class HardwareVideoEncoderFactory implements VideoEncoderFactory {
  ...
  public VideoEncoder createEncoder(VideoCodecInfo input) {
    ...
    int colorFormat = MediaCodecUtils.selectColorFormat(
        SUPPORTED_COLOR_FORMATS, info.getCapabilitiesForType(mime));
    ...
    for (VideoCodecType type : VideoCodecType.values()) {
      ...
      if (codec != null) {
        ...
        if (type != VideoCodecType.H264) {
          supportedCodecInfos.add(new VideoCodecInfo(0, name, properties));
          continue;
        }

        // H264 potentially supports two profiles:  constrained baseline and constrained high.
        properties.put(H264_FMTP_PROFILE_LEVEL_ID, H264_CONSTRAINED_BASELINE_3_1);
        ...
        if (isH264HighProfileSupported(codec)) {
          ...
          properties.put(H264_FMTP_PROFILE_LEVEL_ID, H264_CONSTRAINED_HIGH_3_1);
          properties.put(H264_FMTP_LEVEL_ASYMMETRY_ALLOWED, "1");
          properties.put(H264_FMTP_PACKETIZATION_MODE, "1");
          supportedCodecInfos.add(new VideoCodecInfo(0, name, properties));
        }
  ...
  private boolean isSupportedCodec(MediaCodecInfo info, VideoCodecType type) {
    ...
    if (MediaCodecUtils.selectColorFormat(
            SUPPORTED_COLOR_FORMATS, info.getCapabilitiesForType(type.mimeType()))
        == null) {
      return false;
    }
