...
public class HardwareVideoEncoderFactory implements VideoEncoderFactory {
  ...
  private static class VideoCodecRequirements {
    ...
    VideoCodecRequirements(String codecPrefix, VideoCodecType codecType, int minSdk,
  ...
  private static final VideoCodecRequirements qcomVp8HwProperties =
      new VideoCodecRequirements("OMX.qcom.", VideoCodecType.VP8, Build.VERSION_CODES.KITKAT,
          BitrateAdjustmentType.NO_ADJUSTMENT);
  private static final VideoCodecRequirements exynosVp8HwProperties =
      new VideoCodecRequirements("OMX.Exynos.", VideoCodecType.VP8, Build.VERSION_CODES.M,
          BitrateAdjustmentType.DYNAMIC_ADJUSTMENT);
  private static final VideoCodecRequirements intelVp8HwProperties =
      new VideoCodecRequirements("OMX.Intel.", VideoCodecType.VP8, Build.VERSION_CODES.LOLLIPOP,
          BitrateAdjustmentType.NO_ADJUSTMENT);
  ...
  private static final VideoCodecRequirements qcomVp9HwProperties = new VideoCodecRequirements(
      "OMX.qcom.", VideoCodecType.VP9, Build.VERSION_CODES.N, BitrateAdjustmentType.NO_ADJUSTMENT);
  private static final VideoCodecRequirements exynosVp9HwProperties =
      new VideoCodecRequirements("OMX.Exynos.", VideoCodecType.VP9, Build.VERSION_CODES.N,
  ...
  private static final VideoCodecRequirements qcomH264HwProperties =
      new VideoCodecRequirements("OMX.qcom.", VideoCodecType.H264, Build.VERSION_CODES.KITKAT,
          BitrateAdjustmentType.NO_ADJUSTMENT);
  private static final VideoCodecRequirements exynosH264HwProperties =
      new VideoCodecRequirements("OMX.Exynos.", VideoCodecType.H264, Build.VERSION_CODES.LOLLIPOP,
          BitrateAdjustmentType.FRAMERATE_ADJUSTMENT);
  ...
  private static class VideoCodecProperties {
    ...
    VideoCodecProperties(String name, int colorFormat, VideoCodecRequirements requirements) {
      this.name = name;
  ...
  public VideoEncoder createEncoder(VideoCodecInfo info) {
    ...
    switch (properties.requirements.codecType) {
      case VP8:
    ...
    return new HardwareVideoEncoder(properties.name, properties.requirements.codecType,
        properties.colorFormat, keyFrameIntervalSec, forcedKeyFrameMs, adjuster);
  }
  ...
  private VideoCodecRequirements getMatchingRequirements(MediaCodecInfo codecInfo) {
    ...
    for (VideoCodecRequirements requirements : allSupportedCodecs()) {
      if (codecName.startsWith(requirements.codecPrefix)) {
        if (Build.VERSION.SDK_INT < requirements.minSdk) {
          Logging.w(TAG,
              "Codec " + codecName + " is disabled due to SDK version " + Build.VERSION.SDK_INT);
          continue;
        }
        for (String mimeType : codecInfo.getSupportedTypes()) {
          if (requirements.codecType.mimeType().equals(mimeType)) {
            return requirements;
          }
        }
      }
  ...
  private void loadSupportedCodecs() {
    ...
    for (int i = 0; i < MediaCodecList.getCodecCount(); ++i) {
      ...
      VideoCodecRequirements requirements = getMatchingRequirements(info);
      ...
      try {
        capabilities = info.getCapabilitiesForType(requirements.codecType.mimeType());
      } catch (IllegalArgumentException e) {
      ...
      Logging.d(TAG,
          "Found target encoder for " + requirements.codecType + ": " + name + ". Color: 0x"
              + Integer.toHexString(colorFormat));
      VideoCodecProperties properties = new VideoCodecProperties(name, colorFormat, requirements);
