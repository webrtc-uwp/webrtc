...
- (const uint8_t *)dataY {
  if ([_buffer conformsToProtocol:@protocol(RTCYUVPlanarBuffer)]) {
    return ((id<RTCYUVPlanarBuffer>)_buffer).dataY;
  } else {
    return NULL;
  }
}
...
- (const uint8_t *)dataU {
  if ([_buffer conformsToProtocol:@protocol(RTCYUVPlanarBuffer)]) {
    return ((id<RTCYUVPlanarBuffer>)_buffer).dataU;
  } else {
    return NULL;
  }
}
...
- (const uint8_t *)dataV {
  if ([_buffer conformsToProtocol:@protocol(RTCYUVPlanarBuffer)]) {
    return ((id<RTCYUVPlanarBuffer>)_buffer).dataV;
  } else {
    return NULL;
  }
}
...
- (int)strideY {
  if ([_buffer conformsToProtocol:@protocol(RTCYUVPlanarBuffer)]) {
    return ((id<RTCYUVPlanarBuffer>)_buffer).strideY;
  } else {
    return 0;
  }
}
...
- (int)strideU {
  if ([_buffer conformsToProtocol:@protocol(RTCYUVPlanarBuffer)]) {
    return ((id<RTCYUVPlanarBuffer>)_buffer).strideU;
  } else {
    return 0;
  }
}
...
- (int)strideV {
  if ([_buffer conformsToProtocol:@protocol(RTCYUVPlanarBuffer)]) {
    return ((id<RTCYUVPlanarBuffer>)_buffer).strideV;
  } else {
    return 0;
  }
}
