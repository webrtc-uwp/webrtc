...
rtc::ScopedCFTypeRef<CFDictionaryRef> CreateCFDictionary(CFTypeRef* keys,
                                                         CFTypeRef* values,
                                                         size_t size) {
  return ScopedCF(CFDictionaryCreate(kCFAllocatorDefault,
                                     keys,
                                     values,
                                     size,
...
void SetVTSessionProperty(VTSessionRef session,
  ...
  rtc::ScopedCFTypeRef<CFNumberRef> cfNum =
      ScopedCF(CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &value));
...
void SetVTSessionProperty(VTSessionRef session,
  ...
  rtc::ScopedCFTypeRef<CFNumberRef> cfNum =
      ScopedCF(CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt64Type, &value_64));
  OSStatus status = VTSessionSetProperty(session, key, cfNum.get());
  if (status != noErr) {
    std::string key_string = CFStringToString(key);
    LOG(LS_ERROR) << "VTSessionSetProperty failed to set: " << key_string
...
int H264VideoToolboxEncoder::Encode(
  ...
  rtc::ScopedCFTypeRef<CFDictionaryRef> frame_properties;
...
int H264VideoToolboxEncoder::ResetCompressionSession() {
  ...
  rtc::ScopedCFTypeRef<CFNumberRef> pixel_format =
      ScopedCF(CFNumberCreate(nullptr, kCFNumberLongType, &nv12type));
...
void H264VideoToolboxEncoder::SetEncoderBitrateBps(uint32_t bitrate_bps) {
  if (compression_session_) {
    ...
    rtc::ScopedCFTypeRef<CFNumberRef> bytes_per_second = ScopedCF(CFNumberCreate(
    ...
    rtc::ScopedCFTypeRef<CFNumberRef> one_second =
        ScopedCF(CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt64Type, &one_second_value));
    const void* nums[2] = {bytes_per_second.get(), one_second.get()};
    rtc::ScopedCFTypeRef<CFArrayRef> data_rate_limits =
        ScopedCF(CFArrayCreate(nullptr, nums, 2, &kCFTypeArrayCallBacks));
