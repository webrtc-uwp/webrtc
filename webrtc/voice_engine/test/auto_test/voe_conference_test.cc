...
TEST(VoeConferenceTest, RttAndStartNtpTime) {
  ...
  while (rtc::TimeMillis() < deadline &&
    ...
    if (IsNear(kRttMs, stats_1.roundtrip_time_ms, kRttMs / 10 + 1) &&
        IsNear(kRttMs, stats_2.roundtrip_time_ms, kRttMs / 10 + 1) &&
    ...
    stats_buffer.push(Stats(stats_1.roundtrip_time_ms,
                            stats_2.roundtrip_time_ms,
