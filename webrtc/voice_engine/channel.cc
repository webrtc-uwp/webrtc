...
void Channel::RecordFileEnded(int32_t id) {
  ...
  LOG(LS_VERBOSE) << "Channel::RecordFileEnded() => output file recorder module"
                  << " is shutdown";
}
...
void Channel::Terminate() {
  ...
  if (audio_coding_->RegisterVADCallback(NULL) == -1) {
    LOG(LS_WARNING) << "Terminate() failed to de-register VAD callback"
                    << " (Audio coding module)";
  }
...
int Channel::StartPlayingFileAsMicrophone(const char* fileName,
  ...
  if (!input_file_player_) {
    _engineStatisticsPtr->SetLastError(
        VE_INVALID_ARGUMENT, kTraceError,
        "StartPlayingFileAsMicrophone() filePlayer format is not correct");
    return -1;
  }
...
int Channel::StartPlayingFileAsMicrophone(InStream* stream,
  ...
  if (!input_file_player_) {
    _engineStatisticsPtr->SetLastError(
        VE_INVALID_ARGUMENT, kTraceError,
        "StartPlayingInputFile() filePlayer format is not correct");
    return -1;
  }
...
int Channel::StartRecordingPlayout(const char* fileName,
  ...
  if (!output_file_recorder_) {
    _engineStatisticsPtr->SetLastError(
        VE_INVALID_ARGUMENT, kTraceError,
        "StartRecordingPlayout() fileRecorder format is not correct");
    return -1;
  }
...
int Channel::StopRecordingPlayout() {
  ...
  if (!_outputFileRecording) {
    LOG(LS_ERROR) << "StopRecordingPlayout() is not recording";
    return -1;
  }
...
int32_t Channel::MixOrReplaceAudioWithFile(AudioFrame* audio_input) {
  ...
  const int mixingFrequency = audio_input->sample_rate_hz_;
  {
    ...
    if (!input_file_player_) {
      LOG(LS_WARNING)
          << "Channel::MixOrReplaceAudioWithFile() fileplayer doesn't exist";
      return -1;
    }
