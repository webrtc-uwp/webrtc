...
{
  LOG(LS_INFO) << "" << __FUNCTION__ << " constructed";
...
{
  LOG(LS_INFO) << "" << __FUNCTION__ << " destructed";
  Close();
}
...
int32_t AudioMixerManagerLinuxALSA::CloseSpeaker()
{
  ...
  if (_outputMixerHandle != NULL) {
    ...
    if (errVal < 0) {
      LOG(LS_ERROR) << "     Error detachinging playout mixer: "
                    << LATE(snd_strerror)(errVal) << "";
    }
...
int32_t AudioMixerManagerLinuxALSA::CloseMicrophone()
{
  ...
  if (_inputMixerHandle != NULL) {
    ...
    if (errVal < 0) {
      LOG(LS_ERROR) << "     Error detachinging record mixer: "
                    << LATE(snd_strerror)(errVal) << "";
    }
...
int32_t AudioMixerManagerLinuxALSA::OpenSpeaker(char* deviceName)
{
  ...
  if (_outputMixerHandle != NULL) {
    ...
    LOG(LS_VERBOSE) << "     snd_mixer_attach(_outputMixerHandle, "
                    << controlName << ")";

    errVal = LATE(snd_mixer_attach)(_outputMixerHandle, controlName);
    if (errVal < 0)
    {
      LOG(LS_ERROR) << "     snd_mixer_attach(_outputMixerHandle, "
                    << controlName << ") error: " << LATE(snd_strerror)(errVal)
                    << "";
      _outputMixerHandle = NULL;
      return -1;
    }
    ...
    if (errVal < 0)
    {
      LOG(LS_ERROR) << "     snd_mixer_selem_register(_outputMixerHandle, "
                       "NULL, NULL), error: "
                    << LATE(snd_strerror)(errVal) << "";
      _outputMixerHandle = NULL;
      return -1;
    ...
    if (_outputMixerHandle != NULL)
    {
      LOG(LS_VERBOSE) << "  the output mixer device is now open (0x"
                      << _outputMixerHandle << ")";
    }
...
int32_t AudioMixerManagerLinuxALSA::OpenMicrophone(char *deviceName)
{
  ...
  if (_inputMixerHandle != NULL) {
    ...
    LOG(LS_VERBOSE) << "     snd_mixer_attach(_inputMixerHandle, "
                    << controlName << ")";

    errVal = LATE(snd_mixer_attach)(_inputMixerHandle, controlName);
    if (errVal < 0)
    {
      LOG(LS_ERROR) << "     snd_mixer_attach(_inputMixerHandle, "
                    << controlName << ") error: " << LATE(snd_strerror)(errVal)
                    << "";
    ...
    if (errVal < 0)
    {
      LOG(LS_ERROR) << "     snd_mixer_selem_register(_inputMixerHandle, "
                       "NULL, NULL), error: "
                    << LATE(snd_strerror)(errVal) << "";
    ...
    if (_inputMixerHandle != NULL)
    {
      LOG(LS_VERBOSE) << "  the input mixer device is now open (0x"
                      << _inputMixerHandle << ")";
    }
    ...
    uint32_t& volume) const
{
    ...
    LOG(LS_VERBOSE)
        << "     AudioMixerManagerLinuxALSA::SpeakerVolume() => vol=" << vol
        << "";
...
/*
 ...
 {
 LOG(LS_WARNING)
 << "     Error getting playback volume range: "
 <<  snd_strerror(errVal)
 << "";
 }
 ...
 _outputMixerElement, minVol, maxVol);
 LOG(LS_VERBOSE)
 << "     Playout hardware volume range, min: "
 <<  minVol
 << ", max: "
 <<  maxVol
 << "";
 ...
 {
 LOG(LS_ERROR)
 << "     Error setting playback volume range: "
 <<  snd_strerror(errVal)
 << "";
 return -1;
 }
...
/*
 ...
 {
 LOG(LS_WARNING)
 << "     Error getting capture volume range: "
 <<  snd_strerror(errVal)
 << "";
 }
 ...
 maxVol); LOG(LS_VERBOSE)
 << "     Capture hardware volume range, min: "
 <<  minVol
 << ", max: "
 <<  maxVol
 << "";
 ...
 {
 LOG(LS_ERROR)
 << "     Error setting capture volume range: "
 <<  snd_strerror(errVal)
 << "";
 return -1;
 }
