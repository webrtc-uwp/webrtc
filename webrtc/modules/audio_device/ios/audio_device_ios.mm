...
int32_t AudioDeviceIOS::StopPlayout() {
  ...
  double average_number_of_playout_callbacks_between_glitches = 100000.0;
  ...
  RTC_HISTOGRAM_COUNTS_100000(
      ...
      average_number_of_playout_callbacks_between_glitches);
  RTCLog(@"Average number of playout callbacks between glitches: %f",
         average_number_of_playout_callbacks_between_glitches);
  return 0;
}
...
OSStatus AudioDeviceIOS::OnGetPlayoutData(AudioUnitRenderActionFlags* flags,
  ...
  if (time_stamp->mSampleTime != num_frames) {
    ...
    const int glitch_threshold =
        0.5 * playout_parameters_.GetBufferSizeInMilliseconds() - 1;
