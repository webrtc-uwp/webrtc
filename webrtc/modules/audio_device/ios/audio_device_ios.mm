...
OSStatus AudioDeviceIOS::OnDeliverRecordedData(AudioUnitRenderActionFlags* flags,
  ...
  if (record_audio_buffer_.size() != data_byte_size) {
    record_audio_buffer_.SetSize(data_byte_size);
    LOGI() << "Size of recording buffer: " << record_audio_buffer_.size();
  }
  ...
  // AudioBuffer* audio_buffer = &io_data_ext->mBuffers[0];
  ...
  fine_audio_buffer_->DeliverRecordedData(
      rtc::ArrayView<const int8_t>(data, size_in_bytes),
      kFixedPlayoutDelayEstimate, kFixedRecordDelayEstimate);
  return noErr;
}
