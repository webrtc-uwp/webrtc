...
bool AudioDeviceWindowsCore::CoreAudioIsSupported()
{
  ...
  if (isVistaRTMorXP != 0) {
    LOG(LS_VERBOSE) << "*** Windows Core Audio is only supported on Vista SP1 "
                       "or later => will revert to the Wave API ***";
    return false;
    }
    ...
    if (FAILED(hr))
    {
      LOG(LS_ERROR) << "AudioDeviceWindowsCore::CoreAudioIsSupported() Failed "
                       "to create the required COM object "
                    << hr << "";
      LOG(LS_VERBOSE) << "AudioDeviceWindowsCore::CoreAudioIsSupported() "
                         "CoCreateInstance(MMDeviceEnumerator) failed (hr=0x"
                      << hr << ")";
    ...
    else
    {
        ...
        LOG(LS_VERBOSE) << "AudioDeviceWindowsCore::CoreAudioIsSupported() "
                           "CoCreateInstance(MMDeviceEnumerator) succeeded "
                        << hr << "";
        SAFE_RELEASE(pIMMD);
    }
    ...
    if (MMDeviceIsAvailable)
    {
        ...
        for (uint16_t i = 0; i < numDevsRec; i++)
        {
            ...
            {
              LOG(LS_WARNING)
                  << "AudioDeviceWindowsCore::CoreAudioIsSupported() Failed to "
                     "use Core Audio Recording for device id="
                  << i << "";
            }
        }
        ...
        for (uint16_t i = 0; i < numDevsPlay; i++)
        {
            ...
            if (ok)
            {
              LOG(LS_WARNING)
                  << "AudioDeviceWindowsCore::CoreAudioIsSupported() Failed to "
                     "use Core Audio Playout for device id="
                  << i << "";
            }
        }
    ...
    {
      LOG(LS_VERBOSE) << "*** Windows Core Audio is NOT supported => will "
                         "revert to the Wave API ***";
    }
    ...
    _newMicLevel(0)
{
  ...
  if (!_avrtLibrary) {
    ...
    if (_avrtLibrary) {
      ...
      LOG(LS_VERBOSE) << "AudioDeviceWindowsCore::AudioDeviceWindowsCore() The "
                         "Avrt DLL module is now loaded";
    ...
    if (_avrtLibrary)
    {
        ...
        if (!freeOK)
        {
          LOG(LS_WARNING) << "AudioDeviceWindowsCore::~AudioDeviceWindowsCore()"
                             " failed to free the loaded Avrt DLL module "
                             "correctly";
        }
        else
        {
...
int32_t AudioDeviceWindowsCore::SetPlayoutDevice(uint16_t index)
{
    ...
    {
      LOG(LS_INFO) << "friendly name: \"" << szDeviceName << "\"";
    }
...
int32_t AudioDeviceWindowsCore::InitPlayout()
{
    ...
    if (SUCCEEDED(hr))
    {
      ...
      // format type
      LOG(LS_VERBOSE) << "wFormatTag     : 0x" << pWfxOut->wFormatTag << " ("
                      << pWfxOut->wFormatTag << ")";
    ...
    if (hr == S_OK)
    {
        ...
        LOG(LS_VERBOSE) << "wFormatTag         : 0x" << Wfx.wFormatTag << " ("
                        << Wfx.wFormatTag << ")";
        ...
        LOG(LS_VERBOSE) << "_playBlockSize     : " << _playBlockSize << "";
...
int32_t AudioDeviceWindowsCore::InitRecording()
{
    ...
    if (SUCCEEDED(hr))
    {
      ...
      // format type
      LOG(LS_VERBOSE) << "wFormatTag     : 0x" << pWfxIn->wFormatTag << " ("
                      << pWfxIn->wFormatTag << ")";
      // number of channels (i.e. mono, stereo...)
    ...
    if (hr == S_OK)
    {
        ...
        LOG(LS_VERBOSE) << "wFormatTag        : 0x" << Wfx.Format.wFormatTag
...

    {
        ...
        if (_builtInAecEnabled)
        {
            ...
            if (!_playing)
            {
                ...
                LOG(LS_ERROR) << "Playout must be started before recording "
                                 "when using the built-in AEC";
                return -1;
            }
        }
...
int32_t AudioDeviceWindowsCore::StopPlayout()
{
...

    {
        ...
        if (_builtInAecEnabled && _recording)
        {
            ...
            LOG(LS_WARNING) << "Recording should be stopped before playout "
                               "when using the built-in AEC";
        }
...
DWORD AudioDeviceWindowsCore::DoRenderThread()
{
    ...
    while (keepPlaying)
    {
        ...
        while (keepPlaying)
        {
            ...
            // LOG(LS_VERBOSE) << "#avaliable audio frames = " <<
            // framesAvailable << "";
            ...
            for (uint32_t n = 0; n < n10msBuffers; n++)
            {
                ...
                if (_ptrAudioBuffer)
                {
                    ...
                    {
                        _UnLock();
                        LOG(LS_ERROR) << "output state has been modified "
                                         "during unlocked period";
                        goto Exit;
                    }
...
Exit:
    ...
    if (keepPlaying)
    {
        ...
        _playError = 1;
        LOG(LS_ERROR) << "kPlayoutError message posted: rendering thread has "
                         "ended pre-maturely";
    }
    else
    {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                while (syncBufIndex >= _recBlockSize)
                {
                    if (_ptrAudioBuffer)
                    {
                        ...
                        {
                            _UnLock();
                            LOG(LS_ERROR) << "input state has been modified "
                                             "during unlocked period";
                            goto Exit;
                        }
                    }
                                               ...
                                               int bufferLen)
{
  ...
  if (pDevice != NULL) {
    ...
    if (FAILED(hr)) {
      LOG(LS_ERROR) << "IMMDevice::OpenPropertyStore failed, hr = 0x" << hr
                    << "";
    }
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (FAILED(hr))
        {
          LOG(LS_ERROR) << "IPropertyStore::GetValue failed, hr = 0x" << hr
                        << "";
        }
    }
    ...
    {
        hr = E_FAIL;
        LOG(LS_ERROR) << "IPropertyStore::GetValue returned no value, hr = 0x"
                      << hr << "";
    }
    ...
    if ((SUCCEEDED(hr)) && (VT_LPWSTR != varName.vt))
    {
        ...
        hr = E_UNEXPECTED;
        LOG(LS_ERROR)
            << "IPropertyStore::GetValue returned unexpected type, hr = 0x"
            << hr << "";
    }
...
int32_t AudioDeviceWindowsCore::_EnumerateEndpointDevicesAll(EDataFlow dataFlow) const
{
  ...
  if (count == 0) {
    ...
    for (ULONG i = 0; i < count; i++)
    {
      ...
      hr = pProps->GetValue(PKEY_Device_FriendlyName, &varName);
      CONTINUE_ON_ERROR(hr);
      LOG(LS_INFO) << "friendly name: \"" << varName.pwszVal << "\"";
      ...
      if (dwState & DEVICE_STATE_ACTIVE)
        LOG(LS_VERBOSE) << "state (0x" << dwState << ")  : *ACTIVE*";
      if (dwState & DEVICE_STATE_DISABLED)
        LOG(LS_VERBOSE) << "state (0x" << dwState << ")  : DISABLED";
      if (dwState & DEVICE_STATE_NOTPRESENT)
        LOG(LS_VERBOSE) << "state (0x" << dwState << ")  : NOTPRESENT";
      if (dwState & DEVICE_STATE_UNPLUGGED)
        LOG(LS_VERBOSE) << "state (0x" << dwState << ")  : UNPLUGGED";
      ...
      if (dwHwSupportMask & ENDPOINT_HARDWARE_SUPPORT_VOLUME)
        ...
        LOG(LS_VERBOSE) << "hwmask (0x" << dwHwSupportMask
...
void AudioDeviceWindowsCore::_TraceCOMError(HRESULT hr) const
{
    ...
    LOG(LS_ERROR) << "Core Audio method failed (hr=0x" << hr << ")";
    StringCchPrintf(buf, MAXERRORLENGTH, TEXT("Error details: "));
    StringCchCat(buf, MAXERRORLENGTH, errorText);
