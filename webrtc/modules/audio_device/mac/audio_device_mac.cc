...
int32_t AudioDeviceMac::SetPlayoutDevice(uint16_t index) {
  ...
  LOG(LS_VERBOSE) << "  number of availiable waveform-audio output devices is "
                  << nDevices << "";
...
int32_t AudioDeviceMac::SetRecordingDevice(uint16_t index) {
  ...
  LOG(LS_VERBOSE) << "  number of availiable waveform-audio input devices is "
                  << nDevices << "";
...
int32_t AudioDeviceMac::InitPlayout() {
  ...
  if (_outStreamFormat.mFormatFlags & kAudioFormatFlagIsNonInterleaved) {
    LOG(LS_ERROR) << "Non-interleaved audio data is not "
                     "supported.AudioHardware streams should not have this "
                     "format.";
    return -1;
  }
  ...
  if (_twoDevices) {
    if (_recording && captureDeviceIsAlive == 1) {
      ...
      if (kEventTimeout == _stopEventRec.Wait(2000)) {
        ...
        LOG(LS_WARNING) << " Timed out stopping the capture IOProc. We may "
                           "have failed to detect a device removal.";
    ...
    if (_recording && captureDeviceIsAlive == 1) {
      ...
      if (kEventTimeout == _stopEvent.Wait(2000)) {
        rtc::CritScope critScoped(&_critSect);
        LOG(LS_WARNING) << " Timed out stopping the shared IOProc. We may have "
                           "failed to detect a device removal.";
...
int32_t AudioDeviceMac::StopPlayout() {
  ...
  if (_playing && renderDeviceIsAlive == 1) {
    ...
    if (kEventTimeout == _stopEvent.Wait(2000)) {
      rtc::CritScope critScoped(&_critSect);
      LOG(LS_WARNING) << " Timed out stopping the render IOProc. We may have "
                         "failed to detect a device removal.";
