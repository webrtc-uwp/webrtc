...
void BlockProcessorImpl::ProcessCapture(
  ...
  if (render_buffer_overrun_occurred_) {
    ...
    LOG(LS_VERBOSE) << "Reset due to render buffer overrun";
  }
  ...
  if (render_buffer_underrun) {
    LOG(LS_VERBOSE) << "Render buffer underrun";
  }
  ...
  if (new_delay >= kMinEchoPathDelayBlocks) {
    ...
    render_buffer_->Reset();
    delay_change = true;
    LOG(LS_VERBOSE) << "Reset due to too low delay";
  }
...
void BlockProcessorImpl::BufferRender(
  ...
  if (render_buffer_overrun_occurred_) {
    LOG(LS_VERBOSE) << "Render buffer overrun occurred";
  }
