...
int SimulcastEncoderAdapter::InitEncode(const VideoCodec* inst,
  ...
  for (int i = 0; i < number_of_streams; ++i) {
    ...
    std::unique_ptr<TemporalLayersFactoryAdapter> tl_factory_adapter;
    if (codec_.codecType == webrtc::kVideoCodecVP8) {
      tl_factory_adapter.reset(
          new TemporalLayersFactoryAdapter(i, *codec_.VP8()->tl_factory));
      stream_codec.VP8()->tl_factory = tl_factory_adapter.get();
    }
