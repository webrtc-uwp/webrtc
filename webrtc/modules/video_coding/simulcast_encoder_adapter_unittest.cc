...
class TestVp8SimulcastEncoderAdapter : public TestSimulcast {
 public:
  TestVp8SimulcastEncoderAdapter()
      : TestSimulcast(new SimulcastEncoderAdapter(new Vp8EncoderFactory()),
                      VP8Decoder::Create(),
                      kVideoCodecVP8) {}
...
class TestH264SimulcastEncoderAdapter : public TestSimulcast {
 public:
  TestH264SimulcastEncoderAdapter()
      : TestSimulcast(new SimulcastEncoderAdapter(new H264EncoderFactory()),
                      H264Decoder::Create(),
                      kVideoCodecH264) {}
...
class TestSimulcastEncoderAdapterFake : public ::testing::Test,
 ...
 public:
  ...
  void SetupCodec(VideoCodecType codec_type) {
    const int* temporal_layer_profile =
        (codec_type == kVideoCodecVP8)
            ? static_cast<const int*>(kTestTemporalLayerProfile)
            : nullptr;
    TestSimulcast::DefaultSettings(&codec_, temporal_layer_profile, codec_type);
  ...
  void VerifyCodec(const VideoCodec& ref, int stream_index) {
    ...
    if (ref.codecType == kVideoCodecVP8) {
    ...
    } else if (ref.codecType == kVideoCodecH264) {
  ...
  void InitRefCodec(int stream_index, VideoCodec* ref_codec) {
    *ref_codec = codec_;
    if (codec_.codecType == kVideoCodecVP8) {
  ...
  void VerifyCodecSettings() {
    ...
    ref_codec.qpMax = 45;
    if (ref_codec.codecType == kVideoCodecVP8) {
...
TEST_F(TestSimulcastEncoderAdapterFake, ReusesEncodersInOrderH264) {
  // Set up common settings for three streams.
  TestSimulcast::DefaultSettings(&codec_, nullptr, kVideoCodecH264);
  rate_allocator_.reset(new SimulcastRateAllocator(codec_, nullptr));
  tl_factory_.SetListener(rate_allocator_.get());
  adapter_->RegisterEncodeCompleteCallback(this);
