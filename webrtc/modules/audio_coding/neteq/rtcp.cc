...
void Rtcp::GetStatistics(bool no_reset, RtcpStatistics* stats) {
  ...
  if (received_packets_ == 0) {
    ...
    stats->cumulative_packets_lost = 0;
  } else if (expected_packets > received_packets_) {
    stats->cumulative_packets_lost = expected_packets - received_packets_;
    if (stats->cumulative_packets_lost > 0xFFFFFF) {
      stats->cumulative_packets_lost = 0xFFFFFF;
    }
  } else {
    stats->cumulative_packets_lost = 0;
  }
  ...
  stats->interarrival_jitter = jitter_ >> 4;  // Scaling from Q4.
}
