...
class PacketBuffer {
 public:
  ...
  // Discards all packets that are (strictly) older than timestamp_limit, unless
  // the oldest packets in the buffer is newer than timestamp_limit -
  // horizon_samples. Setting horizon_samples to zero implies that the horizon
  // is set to half the timestamp range. That is, if a packet is more than 2^31
  // timestamps into the future compared with timestamp_limit (including
  // wrap-around), it is considered old.
