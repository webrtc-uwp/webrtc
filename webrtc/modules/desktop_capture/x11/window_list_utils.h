...
// Iterates all on-screen windows in |cache|.display() in decreasing z-order and
// sends them one-by-one to |on_window| function. If |on_window| returns false,
// this function returns immediately. GetWindowList() returns false if native
// APIs failed. If multiple screens are attaching to the |display|, this
// function returns false only when native APIs failed on all screens. Menus,
// panel, minimized windows will be ignored.
