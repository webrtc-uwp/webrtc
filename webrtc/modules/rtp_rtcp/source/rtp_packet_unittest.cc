...
constexpr uint8_t kRtpMediaIdExtensionId = 0xb;
...
TEST(RtpPacketTest, TryToCreateWithEmptyMid) {
  RtpPacketToSend::ExtensionManager extensions;
  extensions.Register<RtpMediaId>(kRtpMediaIdExtensionId);
  RtpPacketToSend packet(&extensions);
  EXPECT_FALSE(packet.SetExtension<RtpMediaId>(""));
}
...
TEST(RtpPacketTest, TryToCreateWithLongMid) {
  ...
  extensions.Register<RtpMediaId>(kRtpMediaIdExtensionId);
  RtpPacketToSend packet(&extensions);
  EXPECT_FALSE(packet.SetExtension<RtpMediaId>(kLongMediaId));
}
...
TEST(RtpPacketTest, ParseWithMid) {
  ...
  std::string mid;
  EXPECT_TRUE(packet.GetExtension<RtpMediaId>(&mid));
