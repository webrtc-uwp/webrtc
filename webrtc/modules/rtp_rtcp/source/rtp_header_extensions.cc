...
bool StringRtpHeaderExtensionSerializer::Parse(
...
bool StringRtpHeaderExtensionSerializer::Write(
    uint8_t* data,
...
bool StringRtpHeaderExtensionSerializer::Parse(
  ...
  // the string. Well-formed string extensions shouldn't contain it.
  str->assign(cstr, strnlen(cstr, data.size()));
  RTC_DCHECK(!str->empty());
  return true;
}

bool StringRtpHeaderExtensionSerializer::Write(uint8_t* data,
