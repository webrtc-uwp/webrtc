...
webrtc::AudioReceiveStream::Stats AudioReceiveStream::GetStats() const {
  ...
  stats.cumulative_packets_lost = call_stats.cumulative_packets_lost;
  ...
  if (codec_inst.plfreq / 1000 > 0) {
    // This calculation will give the wrong result for codecs where the rtp
    // timestamp clock frequency is different from the audio codec sample rate.
    stats.jitter_ms =
        call_stats.interarrival_jitter / (codec_inst.plfreq / 1000);
  }
