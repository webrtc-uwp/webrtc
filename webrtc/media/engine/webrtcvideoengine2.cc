...
class EncoderFactoryAdapter : public webrtc::VideoEncoderFactory {
 public:
  ...
  explicit EncoderFactoryAdapter(
      cricket::WebRtcVideoEncoderFactory* factory,
      const cricket::VideoCodec& codec = VideoCodec(kVp8CodecName))
...
class SoftwareFallbackEncoderFactory
 ...
 public:
  ...
  webrtc::VideoEncoder* CreateVideoEncoder(
    ...
    return new webrtc::VideoEncoderSoftwareFallbackWrapper(codec,
                                                           factory_encoder);
  }
...
class WebRtcSimulcastEncoderFactory
 ...
 public:
  ...
  explicit WebRtcSimulcastEncoderFactory(
      cricket::WebRtcVideoEncoderFactory* factory,
      bool external)
      : factory_(factory), external_factory_(nullptr) {
    if (external) {
      external_factory_ = factory_;
      factory_ = new SoftwareFallbackEncoderFactory(external_factory_);
    }
  }
  ~WebRtcSimulcastEncoderFactory() {
    // If we wrapped an external factory using a s/w fallback factory, delete
    // the s/w fallback factory. We don't own the external factory, so nothing
    // to do there.
    if (external_factory_ && factory_) {
      delete factory_;
    }
  }
 ...
 private:
  cricket::WebRtcVideoEncoderFactory* factory_;
  cricket::WebRtcVideoEncoderFactory* external_factory_;
