...
class PortTest : public testing::Test, public sigslot::has_slots<> {
 ...
 protected:
  ...
  TCPPort* CreateTcpPort(const SocketAddress& addr,
                         PacketSocketFactory* socket_factory) {
    return TCPPort::Create(&main_, socket_factory, &network_, addr.ipaddr(), 0,
  ...
  StunPort* CreateStunPort(const SocketAddress& addr,
    ...
    return StunPort::Create(&main_, factory, &network_, addr.ipaddr(), 0, 0,
                            username_, password_, stun_servers, std::string());
  }
