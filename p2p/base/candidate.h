...
class Candidate {
 public:
  ...
  uint32_t GetPriority(uint32_t type_preference,
    ...
    return (type_preference << 24) | (local_preference << 8) |
           (256 - component_);
  }
...
class CandidateTranslator {
 public:
  ...
  virtual bool GetChannelNameFromComponent(int component,
                                           std::string* channel_name) const = 0;
  virtual bool GetComponentFromChannelName(const std::string& channel_name,
