...
// TODO(eladalon): !!! All of the info logs?
...
// TODO(eladalon): !!! Make sure nobody has piped in a -1 from the hosting app.
const size_t RtcEventLog::kUnlimitedFileSize = 0;
...
class RtcEventLogImpl final : public RtcEventLog {
 public:
  ...
  // TODO(eladalon): !!! Perhaps StartOutput/StopOutput would be better?
...
bool RtcEventLogImpl::StartLogging(const std::string& file_name,
  ...
  RTC_CHECK(max_size_bytes > 0 || max_size_bytes == kUnlimitedFileSize);
  return StartLogging(
      rtc::MakeUnique<RtcEventLogOutputFile>(file_name, max_size_bytes));
}
...
bool RtcEventLogImpl::StartLogging(rtc::PlatformFile platform_file,
  ...
  RTC_CHECK(max_size_bytes > 0 || max_size_bytes == kUnlimitedFileSize);
  return StartLogging(
      rtc::MakeUnique<RtcEventLogOutputFile>(platform_file, max_size_bytes));
}
...
bool RtcEventLogImpl::StartLogging(std::unique_ptr<RtcEventLogOutput> output) {
  ...
  auto output_starter = [this](std::unique_ptr<RtcEventLogOutput> output) {
  ...
  task_queue_.PostTask(rtc::MakeUnique<ResourceOwningTask<RtcEventLogOutput>>(
      std::move(output), output_starter));
...
void RtcEventLogImpl::LogEventsFromMemoryToOutput() {
  ...
  if (appended) {
    ...
    // Successful partial write to the file. Some events could not be written;
