  ...
  [self _addConsumerWithDataLength:2
                          callback:^(SRWebSocket *self, NSData *data) {
                            ...
                            if (!isControlFrame && receivedOpcode != 0 &&
                              ...
                              [self _closeWithProtocolError:@"all data frames after the initial "
                            ...
                            size_t extra_bytes_needed =
                                header.masked ? sizeof(_currentReadMaskKey) : 0;
      ...
      case NSStreamEventEndEncountered: {
        ...
        if (aStream.streamError) {
          ...
          if (!_sentClose && !_failed) {
            _sentClose = YES;
            // If we get closed in this state it's probably not clean because we should be sending
            // this when we send messages
            [self _performDelegateBlock:^{
              if ([self.delegate
                      respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
                [self.delegate webSocket:self
