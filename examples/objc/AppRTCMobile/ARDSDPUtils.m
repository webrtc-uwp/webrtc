...
+ (RTCSessionDescription *)descriptionForDescription:(RTCSessionDescription *)description
  ...
  NSMutableArray *lines =
      [NSMutableArray arrayWithArray:[sdpString componentsSeparatedByString:lineSeparator]];
  ...
  for (NSString *line in lines) {
    ...
    if (codecMatches) {
      [codecPayloadTypes addObject:[line substringWithRange:[codecMatches rangeAtIndex:1]]];
    }
  }
  ...
  NSRange payloadRange = NSMakeRange(kHeaderLength, origMLineParts.count - kHeaderLength);
  NSArray *header = [origMLineParts subarrayWithRange:headerRange];
  NSMutableArray *payloadTypes =
      [NSMutableArray arrayWithArray:[origMLineParts subarrayWithRange:payloadRange]];
