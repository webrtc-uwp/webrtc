...
TraceToStderr::TraceToStderr() : override_time_(false), time_seconds_(0) {
...
TraceToStderr::TraceToStderr(bool override_time)
    : override_time_(override_time), time_seconds_(0) {
  Trace::set_level_filter(kLevelFilter);
  Trace::CreateTrace();
  Trace::SetTraceCallback(this);
}
...
void TraceToStderr::Print(TraceLevel level, const char* msg_array, int length) {
  if (level & kLevelFilter) {
    ...
    if (override_time_) {
      ...
      std::string msg_time =
          msg.substr(Trace::kTimestampPosition, Trace::kTimestampLength);
