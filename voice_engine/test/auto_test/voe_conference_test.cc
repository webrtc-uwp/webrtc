...
TEST(VoeConferenceTest, RttAndStartNtpTime) {
  ...
  while (rtc::TimeMillis() < deadline &&
    ...
    int64_t captured_start_ntp_delay =
        stats_2.capture_start_ntp_time_ms_ - stats_1.capture_start_ntp_time_ms_;
    ...
    stats_buffer.push(
        Stats(stats_1.rttMs, stats_2.rttMs, captured_start_ntp_delay));
  }

  EXPECT_GE(successive_pass, kNeedSuccessivePass)
      << "Expected to get RTT and"
         " start NTP time estimate within 10% of the correct value over "
      << kStatsRequestIntervalMs * kNeedSuccessivePass / 1000 << " seconds.";
  if (successive_pass < kNeedSuccessivePass) {
    printf(
        "The most recent values (RTT for receiver 1, RTT for receiver 2, "
