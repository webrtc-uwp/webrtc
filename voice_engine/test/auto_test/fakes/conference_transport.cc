...
#include "voice_engine/test/auto_test/fakes/conference_transport.h"
...
ConferenceTransport::ConferenceTransport()
  ...
  rtp_header_parser_->RegisterRtpHeaderExtension(
      webrtc::kRtpExtensionAudioLevel, kAudioLevelHeaderId);
  ...
  EXPECT_EQ(0, local_rtp_rtcp_->SetSendAudioLevelIndicationStatus(
                   local_sender_, true, kAudioLevelHeaderId));
...
int ConferenceTransport::GetReceiverChannelForSsrc(
    unsigned int sender_ssrc) const {
    ...
    case Packet::Rtp: {
      ...
      if (rtp_header.ssrc == kLocalSsrc) {
        ...
        if (loudest_filter_.ForwardThisPacket(rtp_header)) {
          ...
          if (destination != -1) {
            local_network_->ReceivedRTPPacket(
                destination, packet.data_, packet.len_, webrtc::PacketTime());
          }
        }
      }
      break;
...
unsigned int ConferenceTransport::AddStream(std::string file_name,
  ...
  EXPECT_EQ(0, remote_rtp_rtcp_->SetSendAudioLevelIndicationStatus(
...
bool ConferenceTransport::RemoveStream(unsigned int id) {
  ...
  EXPECT_EQ(0, remote_network_->DeRegisterExternalTransport(it->second.second));
  EXPECT_EQ(0, local_network_->DeRegisterExternalTransport(it->second.first));
