...
class ExtensionVerifyTransport : public webrtc::Transport {
 public:
  ...
  bool SendRtcp(const uint8_t* data, size_t len) override { return true; }
 ...
 private:
  enum { kPacketsExpected = 10, kSleepIntervalMs = 10 };
...
TEST_F(SendRtpRtcpHeaderExtensionsTest, SentPacketsIncludeAudioLevel) {
  EXPECT_EQ(
      0, voe_rtp_rtcp_->SetSendAudioLevelIndicationStatus(channel_, true, 9));
