...
Statistics::Statistics(uint32_t instanceId)
    : _instanceId(instanceId), _lastError(0), _isInitialized(false) {}
...
int32_t Statistics::SetLastError(int32_t error, TraceLevel level) const {
  WEBRTC_TRACE(level, kTraceVoice, VoEId(_instanceId, -1),
               "error code is set to %d", error);
  rtc::CritScope cs(&lock_);
  _lastError = error;
  return 0;
}
