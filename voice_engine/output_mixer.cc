...
void OutputMixer::PlayNotification(int32_t id, uint32_t durationMs) {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
               "OutputMixer::PlayNotification(id=%d, durationMs=%d)", id,
               durationMs);
  // Not implement yet
}

void OutputMixer::RecordNotification(int32_t id, uint32_t durationMs) {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
               "OutputMixer::RecordNotification(id=%d, durationMs=%d)", id,
               durationMs);
...
int32_t OutputMixer::Create(OutputMixer*& mixer, uint32_t instanceId) {
...
int32_t OutputMixer::SetEngineInformation(voe::Statistics& engineStatistics) {
...
int32_t OutputMixer::SetAudioProcessingModule(
...
int32_t OutputMixer::SetMixabilityStatus(MixerParticipant& participant,
                                         bool mixable) {
  return _mixerModule.SetMixabilityStatus(&participant, mixable);
}

int32_t OutputMixer::SetAnonymousMixabilityStatus(MixerParticipant& participant,
                                                  bool mixable) {
  return _mixerModule.SetAnonymousMixabilityStatus(&participant, mixable);
}

int32_t OutputMixer::MixActiveChannels() {
...
int32_t OutputMixer::DoOperationsOnCombinedSignal(bool feed_data_to_apm) {
