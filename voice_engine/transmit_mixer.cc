  ...
  if (send_typing_noise_warning) {
    ...
    if (_voiceEngineObserverPtr) {
      if (typing_noise_detected) {
        ...
        _voiceEngineObserverPtr->CallbackOnError(-1, VE_TYPING_NOISE_WARNING);
      } else {
        WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
                     "TransmitMixer::OnPeriodicProcess() => "
...
void TransmitMixer::PlayNotification(int32_t id, uint32_t durationMs) {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
               "TransmitMixer::PlayNotification(id=%d, durationMs=%d)", id,
               durationMs);

  // Not implement yet
}

void TransmitMixer::RecordNotification(int32_t id, uint32_t durationMs) {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
               "TransmitMixer::RecordNotification(id=%d, durationMs=%d)", id,
               durationMs);
...
void TransmitMixer::RecordFileEnded(int32_t id) {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
...
int32_t TransmitMixer::Create(TransmitMixer*& mixer, uint32_t instanceId) {
...
int32_t TransmitMixer::SetEngineInformation(ProcessThread& processThread,
...
int32_t TransmitMixer::RegisterVoiceEngineObserver(
...
int32_t TransmitMixer::PrepareDemux(const void* audioSamples,
  ...
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
               ...
               ", "
               "nChannels=%" PRIuS
               ", samplesPerSec=%u, totalDelayMS=%u, "
...
int TransmitMixer::StartPlayingFileAsMicrophone(const char* fileName,
  ...
  WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
               ...
               " startPosition=%d, stopPosition=%d)",
...
int TransmitMixer::StartRecordingMicrophone(const char* fileName,
  ...
  file_recorder_ = FileRecorder::CreateFileRecorder(_fileRecorderId,
...
int32_t TransmitMixer::RecordAudioToFile(uint32_t mixingFrequency) {
  rtc::CritScope cs(&_critSect);
  if (!file_recorder_) {
    WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId, -1),
...
int32_t TransmitMixer::MixOrReplaceAudioWithFile(int mixingFrequency) {
  ...
  if (_mixFileWithMicrophone) {
    ...
    // Currently file stream is always mono.
    // TODO(xians): Change the code when FilePlayer supports real stereo.
    _audioFrame.UpdateFrame(-1, 0xFFFFFFFF, fileBuffer.get(), fileSamples,
                            mixingFrequency, AudioFrame::kNormalSpeech,
                            AudioFrame::kVadUnknown, 1);
  }
  return 0;
}
