...
class WebRtcSessionCreateSDPObserverForTest
 ...
 public:
  ...
  virtual void OnFailure(const std::string& error) { state_ = kFailed; }
 ...
 protected:
  ...
  WebRtcSessionTest()
    ...
    allocator_.reset(new cricket::BasicPortAllocator(
        &network_manager_, stun_servers, SocketAddress(), SocketAddress(),
        SocketAddress()));
  ...
  void Init(
    ...
    session_.reset(new WebRtcSessionForTest(
        &fake_call_, channel_manager_.get(), cricket::MediaConfig(),
        &event_log_, rtc::Thread::Current(), rtc::Thread::Current(),
  ...
  void VerifyCryptoParams(const cricket::SessionDescription* sdp,
    ...
    if (!gcm_enabled) {
      ...
      ASSERT_EQ("AEAD_AES_256_GCM", audio_content->cryptos()[0].cipher_suite);
      EXPECT_EQ(std::string(cricket::kMediaProtocolSavpf),
                audio_content->protocol());
    }
    ...
    if (!gcm_enabled) {
      ...
      ASSERT_EQ("AEAD_AES_256_GCM", video_content->cryptos()[0].cipher_suite);
  ...
  void SetFactoryDtlsSrtp() {
    ...
    std::string identity_name = "WebRTC" + rtc::ToString(rtc::CreateRandomId());
  ...
  void RemoveIceUfragPwdLines(const SessionDescriptionInterface* current_desc,
    ...
    for (; it != contents.end(); ++it) {
      ...
      std::string ufrag_line =
          "a=ice-ufrag:" + transport_desc->ice_ufrag + "\r\n";
      std::string pwd_line = "a=ice-pwd:" + transport_desc->ice_pwd + "\r\n";
      rtc::replace_substrs(ufrag_line.c_str(), ufrag_line.length(), "", 0, sdp);
      rtc::replace_substrs(pwd_line.c_str(), pwd_line.length(), "", 0, sdp);
    }
  }
  ...
  void SetRemoteDescriptionOfferExpectError(const std::string& expected_error,
                                            SessionDescriptionInterface* desc) {
  ...
  void CreateCryptoOfferAndNonCryptoAnswer(
  ...
  void CreateDtlsOfferAndNonDtlsAnswer(
      SessionDescriptionInterface** offer,
  ...
  JsepSessionDescription* ChangeSDPSctpPort(
    ...
    rtc::replace_substrs(default_port_str, strlen(default_port_str),
                         new_port_str, strlen(new_port_str), &offer_str);
    JsepSessionDescription* offer =
        new JsepSessionDescription(offer_basis->type());
    delete offer_basis;
  ...
  JsepSessionDescription* CreateRemoteAnswer(
    ...
    if (!answer->Initialize(
            desc_factory_->CreateAnswer(offer->description(), options, NULL),
  ...
  void TestSessionCandidatesWithBundleRtcpMux(bool bundle, bool rtcp_mux) {
    ...
    if (!rtcp_mux) {
      ...
      rtc::replace_substrs(kRtcpMux.c_str(), kRtcpMux.length(),
                           kXRtcpMux.c_str(), kXRtcpMux.length(), &sdp);
    }
   ...
   public:
    ...
    struct ExpectedBestConnection {
      ExpectedBestConnection(int ipv4_count, int ipv6_count)
          : ipv4_count_(ipv4_count), ipv6_count_(ipv6_count) {}
  ...
  void TestPacketOptions() {
    ...
    media_engine_->GetVideoChannel(0)->SendRtp(test_packet, sizeof(test_packet),
                                               options);
...
TEST_F(WebRtcSessionTest, TestStunError) {
  ...
  AddInterface(rtc::SocketAddress(kClientAddrHost1, kClientAddrPort));
  AddInterface(rtc::SocketAddress(kClientAddrHost2, kClientAddrPort));
  fss_->AddRule(false, rtc::FP_UDP, rtc::FD_ANY,
                rtc::SocketAddress(kClientAddrHost1, kClientAddrPort));
  Init();
  SendAudioVideoStream1();
  InitiateCall();
...
TEST_P(WebRtcSessionTest, TestReceiveNonDtlsOfferWhenDtlsOn) {
  ...
  // SetRemoteDescription will take the ownership of the offer.
  SetRemoteDescriptionOfferExpectError(kSdpWithoutDtlsFingerprint, offer);

  offer = CreateRemoteOffer(options, cricket::SEC_REQUIRED);
  // SetLocalDescription will take the ownership of the offer.
  SetLocalDescriptionOfferExpectError(kSdpWithoutDtlsFingerprint, offer);
}
...
TEST_P(WebRtcSessionTest, TestSetLocalNonDtlsAnswerWhenDtlsOn) {
  ...
  SetRemoteDescriptionWithoutError(offer);
  SetLocalDescriptionAnswerExpectError(kSdpWithoutDtlsFingerprint, answer);
}
...
TEST_P(WebRtcSessionTest, TestSetRemoteNonDtlsAnswerWhenDtlsOn) {
  ...
  // the offer and answer.
  SetLocalDescriptionWithoutError(offer);
  SetRemoteDescriptionAnswerExpectError(kSdpWithoutDtlsFingerprint, answer);
}
...
TEST_F(WebRtcSessionTest, TestSetLocalAnswerWithoutOffer) {
  ...
  SessionDescriptionInterface* answer = CreateRemoteAnswer(offer.get());
...
TEST_F(WebRtcSessionTest, TestSetRemoteAnswerWithoutOffer) {
  ...
  SessionDescriptionInterface* answer = CreateRemoteAnswer(offer.get());
  SetRemoteDescriptionAnswerExpectError("Called in wrong state: STATE_INIT",
                                        answer);
}
...
TEST_F(WebRtcSessionTest, TestDisabledRtcpMuxWithBundleEnabled) {
  ...
  rtc::replace_substrs(rtcp_mux.c_str(), rtcp_mux.length(), xrtcp_mux.c_str(),
                       xrtcp_mux.length(), &offer_str);
...
TEST_F(WebRtcSessionTest, TestIncorrectMLinesInRemoteAnswer) {
  ...
  EXPECT_TRUE(modified_answer->Initialize(answer_copy, answer->session_id(),
  ...
  rtc::replace_substrs(kAudioMline.c_str(), kAudioMline.length(),
                       ...
                       kAudioMlineReplaceStr.length(), &sdp);
...
TEST_F(WebRtcSessionTest, TestSetRemoteOfferFailIfDtlsDisabledAndNoCrypto) {
  ...
  audio->description.identity_fingerprint.reset(
      rtc::SSLFingerprint::CreateFromRfc4572(rtc::DIGEST_SHA_256,
                                             kFakeDtlsFingerprint));
  SetRemoteDescriptionOfferExpectError(kSdpWithoutSdesCrypto, offer);
}
