...
static bool AddStreamParams(MediaType media_type,
  ...
  for (stream_it = streams.begin(); stream_it != streams.end(); ++stream_it) {
...
// collide.
template <class C>
static void FindCodecsToOffer(const std::vector<C>& reference_codecs,
...
static bool CreateMediaContentAnswer(
  ...
  const bool recv_data = answer->type() == cricket::MEDIA_TYPE_DATA;
...
static bool IsDtlsActive(const std::string& content_name,
                         const SessionDescription* current_description) {
  if (!current_description)
    return false;
...
void MediaSessionOptions::AddSendVideoStream(const std::string& id,
                                             const std::string& sync_label,
                                             int num_sim_layers) {
  AddSendStreamInternal(MEDIA_TYPE_VIDEO, id, sync_label, num_sim_layers);
}

void MediaSessionOptions::AddSendStreamInternal(MediaType type,
                                                const std::string& id,
                                                const std::string& sync_label,
                                                int num_sim_layers) {
...
SessionDescription* MediaSessionDescriptionFactory::CreateOffer(
  ...
  GetCodecsToOffer(current_description, supported_audio_codecs, video_codecs_,
                   data_codecs_, &audio_codecs, &video_codecs, &data_codecs);
...
bool MediaSessionDescriptionFactory::AddAudioContentForOffer(
  ...
  if (!CreateMediaContentOffer(
          options, audio_codecs, sdes_policy,
          GetCryptos(GetFirstAudioContentDescription(current_description)),
          crypto_suites, audio_rtp_extensions, add_legacy_, current_streams,
          audio.get())) {
    return false;
  }
...
bool MediaSessionDescriptionFactory::AddVideoContentForOffer(
  ...
  if (!CreateMediaContentOffer(
          options, video_codecs, sdes_policy,
          GetCryptos(GetFirstVideoContentDescription(current_description)),
          crypto_suites, video_rtp_extensions, add_legacy_, current_streams,
          video.get())) {
    return false;
  }
...
bool MediaSessionDescriptionFactory::AddDataContentForOffer(
  ...
  if (!CreateMediaContentOffer(
          options, *data_codecs, sdes_policy,
          GetCryptos(GetFirstDataContentDescription(current_description)),
          crypto_suites, RtpHeaderExtensions(), add_legacy_, current_streams,
          data.get())) {
    return false;
  }
...
bool MediaSessionDescriptionFactory::AddAudioContentForAnswer(
  ...
  if (!CreateMediaContentAnswer(
          ...
          current_streams, add_legacy_, bundle_enabled, audio_answer.get())) {
...
bool MediaSessionDescriptionFactory::AddVideoContentForAnswer(
  ...
  if (!CreateMediaContentAnswer(
          ...
          options, video_codecs_, sdes_policy,
          GetCryptos(GetFirstVideoContentDescription(current_description)),
          video_rtp_extensions_, enable_encrypted_rtp_header_extensions_,
          current_streams, add_legacy_, bundle_enabled, video_answer.get())) {
    return false;
  }
  bool secure = bundle_transport ? bundle_transport->description.secure()
...
bool MediaSessionDescriptionFactory::AddDataContentForAnswer(
  ...
  if (!CreateMediaContentAnswer(
          ...
          options, data_codecs_, sdes_policy,
          GetCryptos(GetFirstDataContentDescription(current_description)),
          RtpHeaderExtensions(), enable_encrypted_rtp_header_extensions_,
          current_streams, add_legacy_, bundle_enabled, data_answer.get())) {
    return false;  // Fails the session setup.
  }
