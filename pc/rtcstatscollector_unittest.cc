...
TEST_F(RTCStatsCollectorTest, CollectRTCCertificateStatsSingle) {
  ...
  EXPECT_CALL(test_->session(), GetStats(_))
  ...
  EXPECT_CALL(test_->session(), GetLocalCertificate(_, _))
      .WillRepeatedly(
  ...
  EXPECT_CALL(test_->session(), GetRemoteSSLCertificate_ReturnsRawPointer(_))
      .WillRepeatedly(Invoke([this, &remote_certinfo](
                                 const std::string& transport_name) {
        if (transport_name == "transport")
...
TEST_F(RTCStatsCollectorTest, CollectRTCCertificateStatsMultiple) {
  ...
  EXPECT_CALL(test_->session(), GetRemoteSSLCertificate_ReturnsRawPointer(_))
      .WillRepeatedly(
          Invoke([this, &audio_remote_certinfo,
                  &video_remote_certinfo](const std::string& transport_name) {
            if (transport_name == "audio") {
