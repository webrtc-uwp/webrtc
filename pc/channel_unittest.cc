...
class ChannelTest : public testing::Test, public sigslot::has_slots<> {
 public:
  ...
  cricket::SessionDescription* CreateSessionDescriptionWithStream(
    ...
    sdesc->AddContent("DUMMY_CONTENT_NAME", cricket::NS_JINGLE_RTP,
                      content.Copy());
    return sdesc;
  }
  ...
  void TestSetContentFailure() {
    ...
    EXPECT_FALSE(
        channel1_->PushdownLocalDescription(&sdesc, cricket::CA_OFFER, &err));
    EXPECT_FALSE(
        channel1_->PushdownLocalDescription(&sdesc, cricket::CA_ANSWER, &err));

    media_channel1_->set_fail_set_send_codecs(true);
    EXPECT_FALSE(
        channel1_->PushdownRemoteDescription(&sdesc, cricket::CA_OFFER, &err));
    media_channel1_->set_fail_set_send_codecs(true);
    EXPECT_FALSE(
        channel1_->PushdownRemoteDescription(&sdesc, cricket::CA_ANSWER, &err));
  }
  ...
  void TestSendTwoOffers() {
    ...
    EXPECT_TRUE(channel1_->PushdownLocalDescription(sdesc1.get(),
    ...
    EXPECT_TRUE(channel1_->PushdownLocalDescription(sdesc2.get(),
                                                    cricket::CA_OFFER, &err));
  ...
  void TestReceiveTwoOffers() {
    ...
    std::unique_ptr<cricket::SessionDescription> sdesc1(
        CreateSessionDescriptionWithStream(1));
    EXPECT_TRUE(channel1_->PushdownRemoteDescription(sdesc1.get(),
                                                     cricket::CA_OFFER, &err));
    EXPECT_TRUE(media_channel1_->HasRecvStream(1));

    std::unique_ptr<cricket::SessionDescription> sdesc2(
        CreateSessionDescriptionWithStream(2));
    EXPECT_TRUE(channel1_->PushdownRemoteDescription(sdesc2.get(),
                                                     cricket::CA_OFFER, &err));
  ...
  void TestSendPrAnswer() {
    ...
    EXPECT_TRUE(channel1_->PushdownLocalDescription(sdesc3.get(),
  ...
  void TestReceivePrAnswer() {
    ...
    std::unique_ptr<cricket::SessionDescription> sdesc3(
        CreateSessionDescriptionWithStream(3));
    EXPECT_TRUE(channel1_->PushdownRemoteDescription(sdesc3.get(),
                                                     cricket::CA_ANSWER, &err));
    EXPECT_TRUE(media_channel1_->HasSendStream(1));
