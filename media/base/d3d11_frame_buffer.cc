...
D3D11VideoFrameBuffer::D3D11VideoFrameBuffer(
  ...
  //TODO: instead of passing desc as a param, call GetDesc here (for color). We don't need it outside.
...
D3D11VideoFrameBuffer::ToI420() {
  ...
  if (rendered_image_desc_.ArraySize == 1) {
    ...
    if (rendered_depth_image_.get() != nullptr) {
      //TODO: copy into depth staging texture
      //If src is depth stencil we always need to copy the full resource because d3d reasons...
      //So for that case we might wanna write extra map code to handle that?
      //Color can stay as is because it works...
    }
  } else {
    RTC_LOG(LS_WARNING)
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
      if (depth_texture_format_ == DXGI_FORMAT_R16_TYPELESS) {
        ...
        RTC_LOG(LS_WARNING) << "Unsupported depth texture format; depth data will not be sent";
      }
