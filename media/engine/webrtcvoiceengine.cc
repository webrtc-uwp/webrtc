...
const int kElevatedTraceFilter =
    kDefaultTraceFilter | webrtc::kTraceStateInfo | webrtc::kTraceInfo;
...
// Set and adjust gain control options.
  ...
  // Turn off the gain control if specified by the field trial. The purpose of
  // the field trial is to reduce the amount of resampling performed inside the
  // audio processing module on mobile platforms by whenever possible turning
  // off the fixed AGC mode and the high-pass filter.
...
void WebRtcVoiceEngine::Print(webrtc::TraceLevel level,
                              const char* trace,
                              int length) {
