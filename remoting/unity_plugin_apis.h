...
#ifndef EXAMPLES_REMOTING_UNITY_PLUGIN_APIS_H_
#define EXAMPLES_REMOTING_UNITY_PLUGIN_APIS_H_
...
typedef void (*I420FRAMEREADY_CALLBACK)(const uint8_t* data_y,
                                        ...
                                        uint32_t height,
                                        void* userData);
typedef void (*LOCALDATACHANNELREADY_CALLBACK)(void* userData);
typedef void (*DATAFROMEDATECHANNELREADY_CALLBACK)(const char* msg, void* userData);
typedef void (*FAILURE_CALLBACK)(const char* msg);
typedef void (*LOCALSDPREADYTOSEND_CALLBACK)(const char* type, const char* sdp, void* userData);
...
WEBRTC_PLUGIN_API int CreatePeerConnectionWithD3D(const char** turn_urls,
                                            const int no_of_urls,
                                            const char* username,
                                            const char* credential,
                                            bool mandatory_receive_video,
...
WEBRTC_PLUGIN_API bool RegisterOnLocalSdpReadytoSend(
    int peer_connection_id,
    LOCALSDPREADYTOSEND_CALLBACK callback, void* userData);
WEBRTC_PLUGIN_API bool RegisterOnIceCandiateReadytoSend(
    int peer_connection_id,
    ICECANDIDATEREADYTOSEND_CALLBACK callback, void* userData);

WEBRTC_PLUGIN_API bool OnD3DFrame(int peer_connection_id, ID3D11Texture2D* frame);
    
// WEBRTC_PLUGIN_API bool StartD3DSource(int peer_connection_id);
}

#endif  // EXAMPLES_REMOTING_UNITY_PLUGIN_APIS_H_
